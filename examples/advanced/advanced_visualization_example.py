...
def create_test_problem():
    ...
    class VisualizationTestProblem(MFGProblem):
        ...
        def rho0(self, x):
            ...
            return np.exp(-20 * (x - 0.2) ** 2) + np.exp(-20 * (x - 0.8) ** 2) + 0.1 * np.exp(-2 * (x - 0.5) ** 2)
...
def generate_sample_data():
    ...
    U = np.sin(2 * np.pi * X) * np.cos(np.pi * X) * np.exp(-2 * T) * (1 + 0.3 * np.sin(4 * np.pi * T))

    # Density: evolving from bimodal to unimodal
    M = (
        np.exp(-10 * (X - 0.3 + 0.2 * T) ** 2)
...
def demo_basic_visualization():
    ...
    backends = []
    try:
        from mfg_pde.utils.advanced_visualization import MATPLOTLIB_AVAILABLE, PLOTLY_AVAILABLE
...
def demo_monitoring_dashboard():
    ...
    try:
        ...
        for i in range(max_iterations):
            ...
            dashboard.add_performance_data("Iteration", iteration_time, {"memory_mb": 150 + 10 * np.random.random()})
...
def demo_animation_features():
    ...
    try:
        ...
        for t in np.linspace(0, 1, n_frames):
            ...
            U_t = np.sin(2 * np.pi * (x_grid - 0.5 * t)) * np.exp(-2 * t) * np.exp(-5 * (x_grid - 0.5) ** 2 * (1 - t))
...
def demo_interactive_report():
    ...
    try:
        ...
        # Solution plot
        solution_fig = visualizer.plot_mfg_solution(U, M, x_grid, t_grid, title="Complete MFG Solution", show=False)
        ...
        convergence_fig = visualizer.plot_convergence_history(
            convergence_data, title="Convergence Analysis", show=False
        )
        ...
        report_path = VisualizationUtils.save_interactive_report(
            results, output_dir / "mfg_analysis_report.html", title="MFG Analysis Report - Advanced Visualization Demo"
        )
...
def demo_solver_integration():
    ...
    try:
        ...
        solver = create_fast_solver(
            problem=problem, solver_type="monitored_particle", collocation_points=collocation_points, num_particles=500
        )
