...
from mfg_pde.factory import MFGModelFactory, MFGModelSpec, create_mfg_model, get_mfg_factory, list_mfg_models
...
def demo_configuration_file_loading():
    ...
    config_data = {
        ...
        "solver": {"type": "fast_solver", "max_iterations": 50, "tolerance": 1e-5},
    }
...
def demo_model_validation():
    ...
    valid_config = {"xmin": 0.0, "xmax": 1.0, "Nx": 51, "T": 1.0, "Nt": 51, "sigma": 0.5, "price_impact": 0.3}
...
def demo_custom_model_registration():
    ...
    from mfg_pde.core.mfg_problem import VALUE_BEFORE_SQUARE_LIMIT, MFGProblem
    ...
    custom_spec = MFGModelSpec(
        ...
        optional_params={"sigma": 1.0, "coefCT": 0.5, "market_power": 1.0, "regulation_strength": 0.5},
    ...
    try:
        custom_model = create_mfg_model(
            "custom_economy", xmin=0, xmax=1, Nx=51, T=1, Nt=51, market_power=1.5, regulation_strength=0.3
        )
...
def main():
    ...
    try:
        ...
        # Summary
        total_models = (
            len(created_models) + len(custom_models) + (1 if config_model else 0) + (1 if custom_registered else 0)
        )
