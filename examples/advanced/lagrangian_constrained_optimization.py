...
def create_social_distancing_problem() -> LagrangianMFGProblem:
    ...
    components = LagrangianComponents(
        ...
        lagrangian_dm_func=lambda t, x, v, m: (
            20.0 * (m - 0.5) if m > 0.5 else 0.2 * m
        ),
        terminal_cost_func=lambda x: 0.0,
        ...
        parameters={
            "capacity_limit": 2.0,
            "distancing_threshold": 0.5,
            "restriction_period": 0.3
        },
...
def create_budget_aware_initial_guess(problem: LagrangianMFGProblem) -> np.ndarray:
    ...
    if problem.components.initial_density_func:
        initial_dist = np.array([
            problem.components.initial_density_func(x) for x in problem.x
        ])
    else:
...
def create_constraint_comparison_plots(solutions: List[Dict]):
    ...
    try:
        fig, axes = plt.subplots(2, len(successful_solutions), 
                               figsize=(4 * len(successful_solutions), 8))
        ...
        for i, solution in enumerate(successful_solutions):
            ...
            if "obstacle" in problem_name.lower():
                ...
                for t in t_grid:
                    ax2.plot([obstacle_center - obstacle_radius, obstacle_center + obstacle_radius], 
                            [t, t], 'r-', linewidth=2, alpha=0.7)
...
def main():
    ...
    try:
        ...
        obstacle_solution = solve_constrained_problem(
            obstacle_problem, "Obstacle Avoidance", max_iterations=25
        )
        solutions.append(obstacle_solution)
        
        # Solve budget-constrained
        budget_solution = solve_constrained_problem(
            budget_problem, "Budget Constrained", max_iterations=25
        )
        solutions.append(budget_solution)
        
        # Solve social distancing
        social_solution = solve_constrained_problem(
            social_problem, "Social Distancing", max_iterations=25
        )
