...
def create_anisotropic_solver_config(
    ...
    cfl_safety_factor: float | None = None,
    ...
    N = max(grid_size)
    if N >= 128:
        enhanced_config.update({"matrix_solver": "sparse_direct", "preconditioner": "ilu", "memory_optimization": True})
    elif N >= 64:
        enhanced_config.update({"matrix_solver": "iterative", "preconditioner": "jacobi"})
...
def create_experiment_solver(problem, solver_config: dict[str, Any] | None = None):
    """
    ...
    if solver_config is None:
        ...
        solver_config = create_anisotropic_solver_config(has_barriers=has_barriers, grid_size=problem.grid_size)
...
def _setup_barrier_handling(solver, problem):
    ...
    def enhanced_cfl_condition(u, m, dx, dy, dt_base):
        ...
        velocity = problem.compute_hamiltonian_gradient(problem._get_grid_points(), np.gradient(u), m.flatten())
        ...
        logger.debug(
            f"CFL components: transport={dt_transport:.2e}, diffusion={dt_diffusion:.2e}, "
    ...
    for i, N in enumerate(grid_sizes):
        ...
        if N >= 128:
            configs[config_name].update(
                {"adaptive_time_stepping": True, "error_tolerance": 1e-8, "max_time_step_refinement": 3}
            )

    return configs


def create_performance_optimized_config(grid_size: tuple, target_accuracy: str = "standard") -> dict[str, Any]:
    """
    ...
    """
    base_config = create_anisotropic_solver_config(has_barriers=True, grid_size=grid_size)
