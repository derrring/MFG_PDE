...
if JAX_AVAILABLE:
    @jax.jit
    def compute_error_indicators_jax(U: jnp.ndarray, M: jnp.ndarray, 
    ...
    @jax.jit
    def conservative_interpolation_jax(old_data: jnp.ndarray, 
                                     old_coords: jnp.ndarray,
                                     new_coords: jnp.ndarray) -> jnp.ndarray:
        """
    ...
    @jax.jit  
    def mass_conservation_enforcement_jax(density: jnp.ndarray, 
                                        target_mass: float) -> jnp.ndarray:
        """
...
if NUMBA_AVAILABLE:
    ...
    def find_cells_to_refine_numba(error_indicators: np.ndarray,
    ...
    @numba_jit(nopython=True, cache=True)
    def traverse_quadtree_numba(node_parents: np.ndarray,    # (N,)
    ...
    @numba_jit(nopython=True, cache=True)
    def collect_mesh_statistics_numba(cell_bounds: np.ndarray,
                                    cell_levels: np.ndarray,
                                    is_leaf: np.ndarray) -> Tuple[int, int, float, float]:
        """
...
def compute_error_indicators_numpy(U: np.ndarray, M: np.ndarray, 
...
def find_cells_to_refine_python(error_indicators: np.ndarray,
...
def create_test_data(size: int = 128) -> Tuple[np.ndarray, np.ndarray, Dict]:
    ...
    U = np.exp(-5 * ((X - 0.5)**2 + (Y - 0.5)**2)) + \
        0.5 * np.exp(-10 * ((X + 0.5)**2 + (Y + 0.5)**2))
    
    M = np.exp(-3 * (X**2 + Y**2)) + \
        0.3 * np.exp(-8 * ((X - 1)**2 + Y**2))
...
def benchmark_error_computation(U: np.ndarray, M: np.ndarray, 
                               dx: float, dy: float, n_runs: int = 10) -> Dict[str, float]:
...
def benchmark_refinement_selection(mesh_data: Dict, error_indicators: np.ndarray,
    ...
    for _ in range(n_runs):
        ...
        candidates_py = find_cells_to_refine_python(
            error_indicators, mesh_data['cell_bounds'], mesh_data['cell_levels'],
            mesh_data['is_leaf'], mesh_data['error_threshold'], mesh_data['max_level']
        )
        times.append(time.time() - start)
...
def main():
    ...
    for size in sizes:
        ...
        if JAX_AVAILABLE:
            ...
            error_indicators_cells = np.random.uniform(0, np.max(error_indicators), 
                                                     len(mesh_data['cell_bounds']))
        else:
    ...
    if JAX_AVAILABLE:
        ...
        error_indicators_cells = np.random.uniform(0, np.max(error_field), 
                                                 len(mesh_data['cell_bounds']))
    ...
    if NUMBA_AVAILABLE:
        ...
        candidates = find_cells_to_refine_numba(
            error_indicators_cells, mesh_data['cell_bounds'], mesh_data['cell_levels'],
            mesh_data['is_leaf'], mesh_data['error_threshold'], mesh_data['max_level']
        )
    ...
    if JAX_AVAILABLE:
        ...
        interpolated = conservative_interpolation_jax(old_data.ravel(), 
