...
from mfg_pde.utils.progress import (
    SolverTimer, IterationProgress, solver_progress, 
    timed_operation, progress_context, check_tqdm_availability
)
from mfg_pde.utils.solver_decorators import (
    with_progress_monitoring, enhanced_solver_method,
    update_solver_progress, format_solver_summary
)
...
def demo_solver_integration():
    ...
    with solver_progress(max_iter, "Manual Integration Demo") as progress:
        for i in range(max_iter):
            ...
            # Update progress
            progress.update(1, error=error, additional_info={
                'iteration': i+1,
            ...
            for i in range(self.max_iterations):
                ...
                if progress_tracker:
                    update_solver_progress(
                        progress_tracker, i+1, error=error,
                        phase="Picard"
                    )
