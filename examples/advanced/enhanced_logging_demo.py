...
def demonstrate_configuration_presets():
    ...
    log_file = configure_research_logging(
        experiment_name="enhanced_logging_demo",
        level="INFO",
        include_debug=False
    )
...
def demonstrate_advanced_logging_functions():
    ...
    log_performance_metric(logger, "HJB equation solve", 1.234, 
                          {"matrix_size": "500x500", "sparsity": "0.02%"})
    log_performance_metric(logger, "Particle update", 0.456,
                          {"num_particles": 10000, "method": "Euler"})
