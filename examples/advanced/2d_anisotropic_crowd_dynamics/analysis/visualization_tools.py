...
class AnisotropicVisualizer:
    ...
    def create_density_evolution_animation(
        self,
        output_file: str = "density_evolution.html",
        ...
        if time_indices is None:
            time_indices = list(range(0, len(self.solution.density_history),
                                    max(1, len(self.solution.density_history) // 20)))
    ...
    def create_velocity_field_plot(
        self,
        time_index: int = -1,
        output_file: str = "velocity_field.png",
        subsample: int = 4
    ):
        """
        ...
        ax2.quiver(X_sub, Y_sub, vx_sub, vy_sub,
    ...
    def create_barrier_influence_analysis(self, output_file: str = "barrier_influence.png"):
        ...
        velocity = -self.problem.compute_hamiltonian_gradient(
            grid_points,
            np.column_stack([grad_u[1].ravel(), grad_u[0].ravel()]),
            m_final.ravel()
        )
    ...
    def create_evacuation_metrics_dashboard(self, output_file: str = "metrics_dashboard.html"):
        ...
        time_grid = getattr(self.solution, 'time_grid',
                          np.linspace(0, self.problem.time_horizon, len(self.solution.density_history)))
        ...
        fig.add_trace(
            go.Scatter(x=time_grid, y=total_mass, mode='lines', name='Total Mass'),
            row=1, col=1
        )
        ...
        # Evacuation progress
        fig.add_trace(
            go.Scatter(x=time_grid, y=evacuation_percentage, mode='lines', name='Evacuated %', line=dict(color='green')),
            row=2, col=1
        )

        # Final density distribution
        fig.add_trace(
            go.Heatmap(x=self.x, y=self.y, z=self.solution.density_history[-1], colorscale='viridis'),
            row=2, col=2
        )

        fig.update_layout(
            title="Evacuation Metrics Dashboard",
            showlegend=False,
            height=600
        )
    ...
    def create_comparative_analysis(
        self,
        other_solutions: Dict[str, Any],
        output_file: str = "comparative_analysis.html"
    ):
        """
        ...
        fig = make_subplots(
            rows=2, cols=len(all_solutions),
            subplot_titles=[f"{label} - Final Density" for label in all_solutions.keys()] +
                          [f"{label} - Evacuation" for label in all_solutions.keys()],
            specs=[[{"type": "heatmap"}] * len(all_solutions),
                   [{"type": "scatter"}] * len(all_solutions)]
        )
        ...
        for i, (label, solution) in enumerate(all_solutions.items()):
            ...
            # Evacuation curve
            time_grid = getattr(solution, 'time_grid',
                              np.linspace(0, self.problem.time_horizon, len(solution.density_history)))
        ...
        fig.update_layout(
            title="Comparative Analysis: Different Configurations",
            height=800
        )
    ...
    def _create_density_snapshots_matplotlib(self, time_indices: Optional[List[int]] = None):
        """Create static density snapshots using matplotlib."""
        if time_indices is None:
            time_indices = [0, len(self.solution.density_history)//3,
                          2*len(self.solution.density_history)//3, -1]
    ...
    def _add_barriers_to_matplotlib_axes(self, axes):
        ...
        for ax in axes:
            for barrier in self.problem.barriers:
                if hasattr(barrier, 'center') and hasattr(barrier, 'radius'):
                    # Circular barrier
                    circle = plt.Circle(barrier.center, barrier.radius,
                                      color='gray', alpha=0.8, zorder=10)
                    ax.add_patch(circle)
                elif hasattr(barrier, 'start') and hasattr(barrier, 'end'):
                    # Linear barrier
                    ax.plot([barrier.start[0], barrier.end[0]],
                           [barrier.start[1], barrier.end[1]],
                           'k-', linewidth=8, alpha=0.8, zorder=10)
