...
class AnisotropicExperiment:
    ...
    def run_convergence_study(self, base_grid: int = 32, max_refinements: int = 3) -> Dict[str, Any]:
        ...
        spatial_result = study.run_spatial_convergence_study(
            barrier_config=self.config['barrier_config']
        )
    ...
    def _run_validation_checks(self, problem: AnisotropicMFGProblem2D, solution: Any) -> Dict[str, Any]:
        ...
        try:
            if hasattr(solution, 'M') and solution.M is not None:
                ...
                mass_variation = np.max(total_mass_history) - np.min(total_mass_history) if len(total_mass_history) > 1 else 0
            else:
    ...
    def _validate_barrier_constraints(self, problem: AnisotropicMFGProblem2D, solution: Any) -> Dict[str, Any]:
        ...
        grid_points = np.column_stack([
            np.linspace(0, 1, problem.Nx1 + 1),
            np.linspace(0, 1, problem.Nx2 + 1)
        ])
...
def main():
    ...
    parser.add_argument("--mode", choices=['single', 'comparative', 'convergence', 'all'],
                       default='single', help="Experiment mode")
    parser.add_argument("--barrier-config", default='anisotropy_aligned',
