...
# MFG_PDE imports
from mfg_pde.core.network_mfg_problem import NetworkMFGProblem, NetworkMFGComponents
...
from mfg_pde.alg.mfg_solvers.network_mfg_solver import (
    create_network_mfg_solver, NetworkFlowMFGSolver
)
...
class NetworkMFGBenchmark:
    ...
    def create_benchmark_networks(self) -> Dict[str, Any]:
        ...
        for name, network in networks.items():
            ...
            self.network_data[name] = {
                'geometry': network,
                'statistics': stats
            }
            print(f"  {name.capitalize()}: {stats['num_edges']} edges, "
    ...
    def run_solver_comparison(self, problem: NetworkMFGProblem, 
    ...
    def _analyze_solution_quality(self, U: np.ndarray, M: np.ndarray, 
                                 problem: NetworkMFGProblem) -> Dict[str, float]:
    ...
    def _compute_total_cost(self, u: np.ndarray, m: np.ndarray, 
                          problem: NetworkMFGProblem) -> float:
    ...
    def _create_plotly_report(self, save_path: str = None) -> go.Figure:
        ...
        fig = make_subplots(
            rows=2, cols=2,
            subplot_titles=['Network Properties', 'Solver Performance', 
                          'Convergence Rates', 'Solution Quality'],
        ...
        for i, solver in enumerate(solver_names):
            ...
            fig.add_trace(go.Bar(
                x=network_names,
                y=times,
                name=f'{solver.capitalize()} Solver',
                offsetgroup=i
            ), row=1, col=2)
        ...
        for i, solver in enumerate(solver_names):
            ...
            fig.add_trace(go.Bar(
                x=network_names,
                y=rates,
                name=f'{solver.capitalize()} Convergence',
                offsetgroup=i
            ), row=2, col=1)
        ...
        fig.update_layout(
            title="Network MFG Benchmark Report",
            height=800,
            showlegend=True
        )
    ...
    def _create_matplotlib_report(self, save_path: str = None) -> plt.Figure:
        ...
        for i, solver in enumerate(solver_names):
            ...
            ax.bar(x_pos + i*width, times, width, label=solver.capitalize(), 
        ...
        for i, solver in enumerate(solver_names):
            ...
            ax.bar(x_pos + i*width, rates, width, label=solver.capitalize(),
                  color=colors[i], alpha=0.7)
        ...
        for network_name, result in results.items():
            ...
            for solver_name, solver_result in result['solver_results'].items():
                if 'error' not in solver_result:
                    ...
                    print(f"    {solver_name:10}: {'✓' if converged else '✗'} "
                          f"({time_taken:.2f}s, error: {error:.2e})")
                else:
