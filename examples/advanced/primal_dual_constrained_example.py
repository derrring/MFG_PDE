...
def solve_with_penalty_method(problem: LagrangianMFGProblem) -> Dict:
    ...
    try:
        solver = VariationalMFGSolver(
            ...
            penalty_weight=1000.0,  # High penalty weight
            use_jax=False
        )
    ...
    try:
        ...
        for i, solution in enumerate(successful_solutions):
            ...
            if hasattr(result, 'cost_history') and result.cost_history:
                ...
                ax3.semilogy(iterations, costs, color=colors[i], linewidth=2, 
                           marker='o', markersize=3, label=f'{method_name} Cost')
            ...
            if method_name == "Primal-Dual Method" and hasattr(result, 'solver_info'):
                ...
                if 'constraint_violation_history' in solver_info:
                    ...
                    if violations:
                        ...
                        ax3_twin.semilogy(iterations_pd, violations, 'g--', linewidth=2,
                                         marker='s', markersize=3, label='Constraint Violations')
        ...
        for i, (method_name, violations) in enumerate(zip(method_names, violation_data)):
            offset = (i - 0.5) * width
            bars = ax4.bar(x_pos + offset, violations, width, label=method_name, 
                          color=colors[i], alpha=0.7)
            
            # Add value labels on bars
            for bar, value in zip(bars, violations):
                height = bar.get_height()
                ax4.text(bar.get_x() + bar.get_width()/2., height + max(violations)*0.01,
