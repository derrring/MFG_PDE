...
# MFG_PDE imports
from mfg_pde import GridNetwork, RandomNetwork, create_grid_mfg_problem, create_random_mfg_problem
...
def create_test_problems() -> Dict[str, Any]:
    ...
    problems['grid'] = create_grid_mfg_problem(width=5, height=5, T=1.0, Nt=20, periodic=False)

    # 2. Random network (25 nodes)
    print("  - Creating random network (25 nodes)...")
    problems['random'] = create_random_mfg_problem(num_nodes=25, connection_prob=0.3, T=1.0, Nt=20, seed=42)
...
def solve_with_multiple_schemes(problem) -> Dict[str, Tuple[np.ndarray, np.ndarray]]:
    ...
    try:
        solver_standard = create_network_mfg_solver(
            problem, solver_type="fixed_point", hjb_solver_type="explicit", fp_solver_type="explicit"
        )
    ...
    try:
        ...
        hjb_solver = create_high_order_network_hjb_solver(problem, scheme="network_upwind", order=2)
    ...
    try:
        solver_lagrangian = create_lagrangian_network_solver(
            problem, velocity_discretization=8, use_relaxed_equilibria=False
        )
...
def demonstrate_3d_visualization(problem, solutions: Dict[str, Tuple[np.ndarray, np.ndarray]]):
    ...
    fig_3d = enhanced_viz.plot_network_3d(
        node_values=terminal_values, title=f"3D Network - Terminal Values - {problem.problem_name}", height_scale=2.0
    )
