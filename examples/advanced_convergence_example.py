...
def demonstrate_convergence_criteria():
    ...
    problem = ExampleMFGProblem(
        xmin=0.0, xmax=1.0, Nx=25,
        T=1.0, Nt=20,
        sigma=0.15,  # Higher volatility to induce more noise
        coefCT=0.03
    )
...
def create_convergence_analysis_plots(solver, diagnostics, save_prefix="convergence"):
    ...
    ax1.axhline(y=solver.convergence_monitor.u_magnitude_tol, 
               color='r', linestyle='--', alpha=0.7, label='Magnitude Tolerance')
    ...
    if wasserstein_history:
        ...
        ax2.axhline(y=solver.convergence_monitor.wasserstein_tol,
                   color='r', linestyle='--', alpha=0.7, label='Wasserstein Tolerance')
    ...
    if valid_means:
        ...
        ax3.axhline(y=solver.convergence_monitor.u_magnitude_tol, 
                   color='r', linestyle='--', alpha=0.7, label='Magnitude Tol')
        ax3.axhline(y=solver.convergence_monitor.u_stability_tol,
                   color='orange', linestyle='--', alpha=0.7, label='Stability Tol')
    ...
    ax4.step(criteria_iterations, [1 if x else 0 for x in wasserstein_ok], 
            'g-', linewidth=2, where='post', label='Wasserstein OK')
    ax4.step(criteria_iterations, [1 if x else 0 for x in u_stabilized_ok], 
            'b-', linewidth=2, where='post', label='U Stabilized OK')
    ax4.step(criteria_iterations, [1 if x else 0 for x in overall_converged], 
            'r-', linewidth=3, where='post', label='Overall Converged')
...
def demonstrate_traditional_vs_advanced_convergence():
    ...
    ax2.step(iterations, traditional_converged.astype(int), 'r-', linewidth=3, 
            where='post', label='Traditional (L2 only)', alpha=0.7)
    ax2.step(iterations, advanced_converged.astype(int), 'g-', linewidth=3,
            where='post', label='Advanced (Multi-criteria)')
