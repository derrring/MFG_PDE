...
from mfg_pde.alg.mfg_solvers import NetworkMFGSolver
from mfg_pde.core import NetworkMFGProblem
...
def create_el_farol_problem(
    ...
    # Create 2-node graph: home (0) <--> bar (1)
    G = nx.Graph()
    G.add_nodes_from([0, 1])  # 0=home, 1=bar
    G.add_edge(0, 1, weight=1.0)  # Agents can move between states

    def local_running_cost(node_id: int, m_node: float, m_all: np.ndarray) -> float:
        """
        Running cost at each node.

        Args:
            node_id: 0 (home) or 1 (bar)
            m_node: Density at this node
            m_all: Density at all nodes

        Returns:
            Running cost (negative of payoff)
        """
        if node_id == 0:  # Home
            return -home_utility  # Cost = -utility
        else:  # Bar (node_id == 1)
            m_bar = m_all[1]  # Attendance fraction at bar
            # Threshold-based payoff with smooth approximation
            epsilon = 0.05
    ...
    def terminal_cost_func(node_id: int) -> float:
    ...
    problem = NetworkMFGProblem(
        ...
        T=T,
        Nt=Nt,
        local_running_cost=local_running_cost,
        terminal_cost=terminal_cost_func,
    )

    # Set initial distribution (start at 50% home, 50% bar)
    m0 = np.array([0.5, 0.5])
...
def plot_results(t_grid, m_traj, u0_traj, u1_traj, threshold=0.6):
    """
    Visualize El Farol Bar dynamics.

    Args:
        t_grid: Time grid
        m_traj: Bar attendance trajectory m(t)
        u0_traj: Value function for staying home
        u1_traj: Value function for going to bar
    ...
    ax = axes[0, 0]
    ax.plot(t_grid, m_traj, "b-", linewidth=2, label="Bar attendance m(t)")
    ...
    ax = axes[0, 1]
    ax.plot(t_grid, u0_traj, "g-", linewidth=2, label="u₀(t) [home]")
    ax.plot(t_grid, u1_traj, "b-", linewidth=2, label="u₁(t) [bar]")
    ax.set_xlabel("Time t", fontsize=12)
    ...
    ax = axes[1, 1]
    sc = ax.scatter(m_traj, value_diff, c=t_grid, cmap="viridis", s=20, alpha=0.7)
...
def main():
    ...
    logger.info(f"  Payoff: {10.0} (good) vs {-5.0} (bad)")
    logger.info("  Graph: 2 nodes (home <--> bar)")
    ...
    m_traj = result.density_trajectory[:, 1]  # Fraction at bar (node 1)
    u0_traj = result.value_function[:, 0]  # Value of home (node 0)
    u1_traj = result.value_function[:, 1]  # Value of bar (node 1)

    # Summary statistics
    logger.info("\n[3/3] Analysis:")
    mean_attendance = np.mean(m_traj)
    time_above_threshold = np.mean(m_traj > 0.6)
    logger.info(f"  Mean attendance: {mean_attendance:.1%}")
    logger.info(f"  Time overcrowded: {time_above_threshold:.1%}")
    logger.info(f"  Attendance range: [{m_traj.min():.1%}, {m_traj.max():.1%}]")

    # Visualize
    logger.info("\nGenerating visualizations...")
    fig = plot_results(t_grid, m_traj, u0_traj, u1_traj, threshold=0.6)
