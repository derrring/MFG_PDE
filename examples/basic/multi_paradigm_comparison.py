...
from mfg_pde.alg.neural import TORCH_AVAILABLE, MFGPINNSolver

# Multi-paradigm imports - new structure
from mfg_pde.alg.numerical import ParticleCollocationSolver
...
def solve_numerical_paradigm(problem: ExampleMFGProblem) -> dict[str, Any]:
    ...
    solver = ParticleCollocationSolver(
        mfg_problem=problem, max_iterations=50, convergence_tolerance=1e-4, damping_factor=0.7
    )
...
def solve_optimization_paradigm(problem: ExampleMFGProblem) -> dict[str, Any]:
    ...
    solver = VariationalMFGSolver(mfg_problem=problem, max_iterations=30, convergence_tolerance=1e-4, step_size=0.01)
...
def compare_solutions(results: list) -> None:
    ...
    if len(valid_results) >= 2:
        ...
        for result in valid_results:
            ...
            if result.get("final_residual"):
                print(f"  PDE residual: {result['final_residual']:.2e}")
            if result.get("final_objective"):
...
def visualize_paradigm_comparison(results: list) -> None:
    ...
    _fig, axes = plt.subplots(1, len(valid_results), figsize=(5 * len(valid_results), 4))
...
def main():
    ...
    try:
        ...
        results.append({"paradigm": "Numerical", "method": "Failed", "solution": None, "error": str(e)})
    ...
    try:
        ...
        results.append({"paradigm": "Optimization", "method": "Failed", "solution": None, "error": str(e)})
