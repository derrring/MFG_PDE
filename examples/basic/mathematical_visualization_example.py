...
def generate_analytical_solution():
    ...
    U = np.exp(-alpha * T) * (np.sin(np.pi * X) + beta * np.sin(3 * np.pi * X)) + gamma * (X - 0.5) ** 2 * (1 - 0.5 * T)
...
def demo_hjb_mathematical_analysis():
    ...
    try:
        ...
        quick_hjb_fig = quick_hjb_analysis(U, x_grid, t_grid, save_path=output_dir / "hjb_quick_analysis.png")
...
def demo_fokker_planck_analysis():
    ...
    try:
        ...
        # Quick analysis
        quick_fp_fig = quick_fp_analysis(M, x_grid, t_grid, save_path=output_dir / "fp_quick_analysis.png")
...
def demo_phase_space_analysis():
    ...
    try:
        ...
        quick_phase_fig = quick_phase_space_analysis(
            U, M, x_grid, t_grid, save_path=output_dir / "quick_phase_space.png"
        )
...
def demo_real_solver_mathematical_analysis():
    ...
    try:
        ...
        solver = create_fast_solver(
            problem=problem, solver_type="monitored_particle", collocation_points=collocation_points, num_particles=800
        )
