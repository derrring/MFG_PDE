...
from mfg_pde.geometry import BoundaryManager, Domain2D, MeshManager, MeshPipeline
from mfg_pde.utils.logging import configure_basic_logging, get_logger
...
def demo_simple_rectangle():
    ...
    domain = Domain2D(domain_type="rectangle", bounds=(0.0, 2.0, 0.0, 1.0), mesh_size=0.1)  # (xmin, xmax, ymin, ymax)

    # Create and execute pipeline
    pipeline = MeshPipeline(domain, output_dir="./output/rectangle")
    mesh_data = pipeline.execute_pipeline(
        stages=["generate", "analyze", "export"], export_formats=["msh", "vtk", "xdmf"]
    )

    # Print summary
    logger.info(f"Generated mesh: {mesh_data.num_vertices} vertices, " f"{mesh_data.num_elements} triangles")
...
def demo_domain_with_holes():
    ...
    domain = Domain2D(domain_type="rectangle", bounds=(0.0, 2.0, 0.0, 1.0), holes=holes, mesh_size=0.05)

    # Execute pipeline
    pipeline = MeshPipeline(domain, output_dir="./output/with_holes")
    mesh_data = pipeline.execute_pipeline()

    logger.info(f"Mesh with holes: {mesh_data.num_vertices} vertices, " f"{mesh_data.num_elements} triangles")
...
def demo_circular_domain():
    ...
    domain = Domain2D(domain_type="circle", center=(0.0, 0.0), radius=1.0, mesh_size=0.1)

    # Execute pipeline
    pipeline = MeshPipeline(domain, output_dir="./output/circle")
    mesh_data = pipeline.execute_pipeline()

    logger.info(f"Circular mesh: {mesh_data.num_vertices} vertices, " f"{mesh_data.num_elements} triangles")
...
def demo_polygon_domain():
    ...
    domain = Domain2D(domain_type="polygon", vertices=vertices, mesh_size=0.08)

    # Execute pipeline
    pipeline = MeshPipeline(domain, output_dir="./output/polygon")
    mesh_data = pipeline.execute_pipeline()

    logger.info(f"L-shaped mesh: {mesh_data.num_vertices} vertices, " f"{mesh_data.num_elements} triangles")
...
def demo_boundary_conditions():
    ...
    domain = Domain2D(domain_type="rectangle", bounds=(0.0, 1.0, 0.0, 1.0), mesh_size=0.1)
    ...
    boundary_manager.add_boundary_condition(
        region_id=1, bc_type="dirichlet", value=0.0, description="Bottom boundary: u = 0"
    )

    # Right boundary (region 2): Neumann du/dn = 1
    boundary_manager.add_boundary_condition(
        region_id=2, bc_type="neumann", gradient_value=1.0, description="Right boundary: du/dn = 1"
    )
    ...
    boundary_manager.add_boundary_condition(
        region_id=4, bc_type="neumann", gradient_value=0.0, description="Left boundary: no flux"
    )
...
def demo_mesh_quality_analysis():
    ...
    coarse_domain = Domain2D(domain_type="rectangle", bounds=(0.0, 1.0, 0.0, 1.0), mesh_size=0.2)

    fine_domain = Domain2D(domain_type="rectangle", bounds=(0.0, 1.0, 0.0, 1.0), mesh_size=0.05)
...
def demo_batch_processing():
    ...
    geometries = {
        "rect_coarse": {"type": "rectangle", "bounds": (0.0, 1.0, 0.0, 1.0), "mesh_size": 0.15},
        "rect_fine": {"type": "rectangle", "bounds": (0.0, 1.0, 0.0, 1.0), "mesh_size": 0.05},
        "circle": {"type": "circle", "kwargs": {"center": (0.5, 0.5), "radius": 0.4}, "mesh_size": 0.08},
    }
    ...
    results = manager.batch_generate_meshes(list(geometries.keys()), stages=["generate", "analyze"])
    ...
    for name, mesh_data in results.items():
        logger.info(f"  {name}: {mesh_data.num_vertices} vertices, " f"{mesh_data.num_elements} elements")
