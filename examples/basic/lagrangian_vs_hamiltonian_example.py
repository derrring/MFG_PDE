...
def create_test_problems():
    ...
    lagrangian_problem = create_quadratic_lagrangian_mfg(
        xmin=xmin, xmax=xmax, Nx=Nx, T=T, Nt=Nt, kinetic_coefficient=1.0, congestion_coefficient=0.5, sigma=sigma
    )
...
def solve_lagrangian_formulation(problem):
    ...
    try:
        ...
        result = solver.solve(max_iterations=50, tolerance=1e-4, verbose=True)
