...
class AMRMemoryProfiler:
    ...
    def estimate_theoretical_memory_requirements(self, problem_size: Tuple[int, ...], 
                                               amr_enabled: bool = False,
                                               max_refinement_levels: int = 5) -> float:
    ...
    def profile_memory_scaling(self):
        ...
        for size in problem_sizes:
            ...
            solver_configs = [
                ...
                },
                {
                    ...
                    'create_solver': lambda p: create_amr_solver(
                        p, base_solver_type='fixed_point',
                        error_threshold=1e-4, max_levels=4,
            ...
            for config in solver_configs:
                ...
                peak_memory = max([initial_snapshot.process_memory_mb] + memory_timeline + [final_snapshot.process_memory_mb])
                
                # Estimate theoretical minimum
                theoretical_min = self.estimate_theoretical_memory_requirements(
                    (size,), config['amr_enabled'], 4
                )
    ...
    def profile_amr_memory_components(self):
        ...
        problem = ExampleMFGProblem(
            T=0.5, xmin=0.0, xmax=1.0,
            Nx=256, Nt=20,
            sigma=0.05,  # Sharp features for AMR
            coefCT=2.0
        )
        problem.domain = domain
        problem.dimension = 1
        ...
        solver_memory = checkpoints['solver_created'].process_memory_mb - checkpoints['problem_created'].process_memory_mb
    ...
    def generate_memory_plots(self):
        ...
        try:
            ...
            if scaling_results:
                ...
                for size in problem_sizes:
                    uniform_res = [r for r in scaling_results if r.problem_size[0] == size and 'Uniform' in r.solver_type]
                ...
                if uniform_efficiency:
                    ax.plot(problem_sizes[:len(uniform_efficiency)], uniform_efficiency, 'b-o', label='Uniform', markersize=6)
                if amr_efficiency:
            ...
            if leak_results and leak_results[0].allocation_pattern:
                ...
                if len(pattern) > 1:
                    ...
                    p = np.poly1d(z)
                    ax.plot(iterations, p(iterations), "r--", alpha=0.8, 
                           label=f'Trend: {z[0]:.2f} MB/iter')
                    ax.legend()
            ...
            if component_results:
                ...
                components = ['Mesh\nStructure', 'Solution\nData', 'Overhead']
                memory_values = [
                    result.mesh_memory_mb,
                    result.solution_memory_mb,
                    result.overhead_memory_mb
                ]
        ...
        with open(report_file, 'w') as f:
            ...
            if scaling_results:
                ...
                for result in scaling_results:
                    f.write(f"| {result.problem_size[0]} | {result.solver_type} | "
                ...
                if len(amr_results) >= 2:
                    ...
                    f.write(f"**AMR Scaling**: {size_ratio:.1f}x size increase → "
                           f"{memory_ratio:.1f}x memory increase  \n\n")
            ...
            if scaling_results:
                avg_uniform_efficiency = np.mean([r.memory_efficiency for r in uniform_results if r.memory_efficiency > 0])
                ...
                if avg_amr_efficiency > avg_uniform_efficiency:
                    ...
                    f.write(f"**Uniform Advantage**: {avg_uniform_efficiency/avg_amr_efficiency:.2f}x more efficient  \n\n")
