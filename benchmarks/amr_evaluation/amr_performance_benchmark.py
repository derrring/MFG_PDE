...
from mfg_pde.geometry import Domain1D, periodic_bc, dirichlet_bc
...
class BenchmarkResult:
    ...
    max_refinement_level: int = None
    total_adaptations: int = None
    mesh_efficiency_ratio: float = None  # AMR elements / uniform elements
    
    # Solution accuracy (vs reference)
    l2_error_u: float = None
    l2_error_m: float = None
    max_error_u: float = None
...
class AMRBenchmarkSuite:
    ...
    def __init__(self, output_dir: str = "benchmark_results"):
        ...
        print(f"AMR Performance Benchmarking Suite")
    ...
    def benchmark_problem(self, 
                         problem_generator,
                         problem_name: str,
                         solver_configs: List[Dict[str, Any]],
                         reference_solver_config: Optional[Dict[str, Any]] = None) -> List[BenchmarkResult]:
        """
        ...
        for i, config in enumerate(solver_configs):
            ...
            result = self._benchmark_single_config(
                problem, problem_name, config, reference_solution
            )
    ...
    def _benchmark_single_config(self, 
                                problem, 
                                problem_name: str, 
                                config: Dict[str, Any],
                                reference_solution = None) -> BenchmarkResult:
        ...
        if amr_enabled and isinstance(result, dict):
            if 'mesh_statistics' in result:
                ...
                total_elements = mesh_stats.get('total_intervals', 
                                mesh_stats.get('total_triangles',
                                mesh_stats.get('total_cells', None)))
    ...
    def run_comprehensive_benchmark(self):
        ...
        def create_1d_congestion_problem(nx=64):
            ...
            problem = ExampleMFGProblem(
                T=1.0, xmin=0.0, xmax=2.0, 
                Nx=nx, Nt=50,
                sigma=0.05,  # Sharp features
        ...
        for nx in [32, 64, 128]:
            ...
            self.benchmark_problem(
                lambda: create_1d_congestion_problem(nx),
                f"1D_Congestion_N{nx}",
                solver_configs
            )
        ...
        def create_2d_problem():
            problem = ExampleMFGProblem(
                T=1.0, xmin=0.0, xmax=1.0, ymin=0.0, ymax=1.0,
                Nx=32, Ny=32, Nt=30,
                sigma=0.1, coefCT=1.5
            )
            problem.dimension = 2
            return problem
    ...
    def generate_performance_report(self):
        ...
        with open(report_file, 'w') as f:
            ...
            for result in self.results:
                ...
                elements_str = str(result.total_elements) if result.total_elements else f"{np.prod(result.problem_size)}"
                
                f.write(f"| {result.problem_name} | {result.solver_type} | "
            ...
            if amr_results and uniform_results:
                ...
                for amr_result in amr_results:
                    ...
                    for uniform in uniform_results:
                        if (uniform.problem_name.replace('Uniform-', '').replace('AMR-', '') == 
                    ...
                    if uniform_result:
                        ...
                        f.write(f"- **Mesh Efficiency**: {amr_result.mesh_efficiency_ratio:.3f} "
            ...
            if amr_results:
                ...
                if avg_efficiency < 0.7:
                    f.write("✅ **AMR Effective**: Average mesh reduction of "
                           f"{100*(1-avg_efficiency):.1f}% demonstrates significant efficiency gains.\n\n")
                else:
                    f.write("⚠️ **AMR Limited Benefit**: Consider adjusting error thresholds or problem characteristics.\n\n")
