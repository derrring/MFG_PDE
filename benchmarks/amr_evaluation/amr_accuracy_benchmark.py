...
class AMRAccuracyBenchmark:
    ...
    def compute_solution_errors(self, 
        ...
        if len(U_comp) > 2:
            ...
            h1_error_u = np.sqrt(
                np.trapz((U_comp - U_ref)**2, dx=dx) + 
                np.trapz((du_comp - du_ref)**2, dx=dx)
            )
        else:
    ...
    def benchmark_manufactured_solution(self):
        ...
        for nx in grid_sizes:
            ...
            configs = [
                ...
                },
                {
                    ...
                    'create_solver': lambda p: create_amr_solver(
                        ...
                        error_threshold=1e-5, max_levels=4
                    ),
                    'amr_enabled': True
                }
            ]
            ...
            for config in configs:
                ...
                errors = self.compute_solution_errors(
                    (U, M), (U_ref_interp, M_ref_interp), x_comp
                )
    ...
    def benchmark_sharp_features(self):
        ...
        configs = [
            ...
            },
            {
                ...
                'create_solver': lambda p: create_amr_solver(
                    p, base_solver_type='fixed_point',
                    error_threshold=1e-3, max_levels=3
                ),
                'amr_enabled': True
            },
            {
                ...
                'create_solver': lambda p: create_amr_solver(
                    p, base_solver_type='fixed_point',
                    error_threshold=1e-4, max_levels=5
                ),
                'amr_enabled': True
            },
            {
                ...
                'create_solver': lambda p: create_amr_solver(
                    p, base_solver_type='fixed_point',
                    error_threshold=1e-5, max_levels=6
                ),
                'amr_enabled': True
            }
        ]
    ...
    def generate_accuracy_report(self):
        ...
        with open(report_file, 'w') as f:
            ...
            for result in self.results:
                f.write(f"| {result.problem_name} | {result.solver_name} | "
