...
class AMRGPUProfiler:
    ...
    def profile_jax_compilation_overhead(self, problem_sizes: List[int]):
        ...
        def mock_error_estimation(u_vals, m_vals, dx):
            ...
            # Error indicator
            error_indicator = dx * (jnp.abs(du_dx) + jnp.abs(dm_dx)) + \
                            dx**2 * (jnp.abs(d2u_dx2) + jnp.abs(d2m_dx2))
    ...
    def generate_performance_plots(self):
        ...
        try:
            ...
            plt.tight_layout()
            plt.savefig(self.output_dir / 'gpu_performance_analysis.png', 
