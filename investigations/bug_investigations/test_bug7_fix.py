...
def test_bug7_time_index_fix():
    """
    Test Bug #7 Fix: Time-Index Correction in HJB-MFG Coupling

    Fix: Changed base_hjb.py:763 from M[n+1] to M[n]

    Expected: Picard iterations should converge with monotonic error decrease.
    """
    print()
    print("=" * 70)
    ...
    # Assert test passed
    assert result.converged, f"Solver should converge (got {result.iterations} iterations)"
    assert result.iterations < 50, f"Should converge quickly (got {result.iterations} iterations)"
