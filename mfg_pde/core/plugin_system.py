...
class SolverPlugin(ABC):
    ...
    @abstractmethod
    def create_solver(self, problem, solver_type: str, config: Optional[MFGSolverConfig] = None, **kwargs):
...
class PluginManager:
    ...
    def _create_core_solver(self, problem, solver_type: str, config: Optional[MFGSolverConfig] = None, **kwargs):
        ...
        for plugin_file in path.glob("*.py"):
            ...
            try:
                ...
                for name, obj in inspect.getmembers(module, inspect.isclass):
                    if (issubclass(obj, (SolverPlugin, AnalysisPlugin)) and 
                        obj not in (SolverPlugin, AnalysisPlugin)):
...
def discover_and_load_plugins(search_paths: Optional[List[Path]] = None) -> Dict[str, bool]:
