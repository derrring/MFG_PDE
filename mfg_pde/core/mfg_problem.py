...
class MFGProblem:
    ...
    def _init_1d_legacy(
        ...
        geometry = TensorProductGrid(dimension=1, bounds=[(xmin_scalar, xmax_scalar)], num_points=[Nx_scalar + 1])
        dx = geometry.get_mesh_spacing()[0]
    ...
    def _init_nd(
        ...
        # Create TensorProductGrid geometry object for all dimensions
        from mfg_pde.geometry import TensorProductGrid

        # Convert spatial_discretization (intervals) to num_points (grid points = intervals + 1)
        ...
        # TensorProductGrid.get_grid_shape() returns the proper shape for all dimensions
        self.spatial_shape = geometry.get_grid_shape()
