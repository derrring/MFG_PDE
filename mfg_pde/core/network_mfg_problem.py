...
class NetworkMFGProblem(MFGProblem):
    ...
    def hamiltonian(self, node: int, neighbors: List[int], m: np.ndarray, 
                   p: np.ndarray, t: float) -> float:
        """
    ...
    def _default_network_hamiltonian(self, node: int, neighbors: List[int], 
                                   m: np.ndarray, p: np.ndarray, t: float) -> float:
    ...
    def hamiltonian_dm(self, node: int, neighbors: List[int], m: np.ndarray, 
                      p: np.ndarray, t: float) -> float:
    ...
    def get_initial_density(self) -> np.ndarray:
        ...
        if self.components.initial_node_density_func is not None:
            return np.array([self.components.initial_node_density_func(i) 
    ...
    def get_terminal_value(self) -> np.ndarray:
        ...
        if self.components.terminal_node_value_func is not None:
            return np.array([self.components.terminal_node_value_func(i) 
                           for i in range(self.num_nodes)])
    ...
    def apply_graph_laplacian(self, u: np.ndarray, 
                             coefficient: float = 1.0) -> np.ndarray:
        """
    ...
    def __str__(self) -> str:
        ...
        return (f"NetworkMFGProblem({self.problem_name})\n"
...
def create_grid_mfg_problem(width: int, 
                           height: int = None,
                           T: float = 1.0,
                           Nt: int = 100,
                           periodic: bool = False,
                           **kwargs) -> NetworkMFGProblem:
...
def create_random_mfg_problem(num_nodes: int,
...
def create_scale_free_mfg_problem(num_nodes: int,
