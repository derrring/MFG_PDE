...
from .domain_1d import Domain1D, BoundaryConditions
...
class OneDimensionalAMRMesh:
    ...
    def _setup_jax_functions(self):
        ...
        @jit
        def conservative_interpolation_1d(parent_vals: jnp.ndarray, 
                                        child_coords: jnp.ndarray,
                                        parent_coords: jnp.ndarray) -> jnp.ndarray:
    ...
    def _estimate_interval_error(self, interval: Interval1D,
                               solution_data: Dict[str, np.ndarray],
                               error_estimator: BaseErrorEstimator) -> float:
        """
    ...
    def get_grid_points(self) -> Tuple[np.ndarray, np.ndarray]:
        ...
        sorted_intervals = sorted(
            [self.intervals[iid] for iid in self.leaf_intervals],
            key=lambda i: i.x_min
        )
...
def create_1d_amr_mesh(domain_1d: Domain1D,
                      ...
                      max_levels: int = 5,
                      backend: Optional[BaseBackend] = None) -> OneDimensionalAMRMesh:
    """
