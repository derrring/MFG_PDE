...
class BoundaryConditions:
    ...
    def validate_values(self):
        ...
        if self.type == "dirichlet":
            ...
            required_params = [
                self.left_alpha, self.left_beta, self.left_value,
                self.right_alpha, self.right_beta, self.right_value
            ]
    ...
    def __str__(self) -> str:
        ...
        if self.type == "periodic":
            ...
            return f"Robin(left: {self.left_alpha}u + {self.left_beta}u' = {self.left_value}, " \
...
class Domain1D:
    ...
    def __repr__(self) -> str:
        ...
        return f"Domain1D(xmin={self.xmin}, xmax={self.xmax}, " \
...
def neumann_bc(left_gradient: float, right_gradient: float) -> BoundaryConditions:
    ...
    return BoundaryConditions(
        type="neumann", 
        left_value=left_gradient, 
        right_value=right_gradient
    )
...
def robin_bc(left_alpha: float, left_beta: float, left_value: float,
             right_alpha: float, right_beta: float, right_value: float) -> BoundaryConditions:
