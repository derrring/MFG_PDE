...
def modern_plot_mfg_solution(U: np.ndarray, M: np.ndarray, x_grid: np.ndarray, 
                            t_grid: np.ndarray, title: str = "MFG Solution",
...
def modern_plot_convergence(convergence_data: Dict[str, List[float]], 
    ...
    for i, (metric_name, values) in enumerate(convergence_data.items()):
        iterations = list(range(1, len(values) + 1))
        ax.semilogy(iterations, values, 'o-', label=metric_name, 
                   color=colors[i], linewidth=2, markersize=4)
        
        # Add tolerance line if provided
        if tolerances and metric_name in tolerances:
            ax.axhline(y=tolerances[metric_name], color=colors[i], 
                      linestyle='--', alpha=0.7, 
