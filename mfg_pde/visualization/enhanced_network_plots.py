...
class EnhancedNetworkMFGVisualizer(NetworkMFGVisualizer):
    ...
    def plot_lagrangian_trajectories(self,
                                   trajectories: List[List[int]],
                                   U: Optional[np.ndarray] = None,
                                   M: Optional[np.ndarray] = None,
                                   title: str = "Lagrangian Trajectories",
                                   interactive: bool = True,
                                   save_path: Optional[str] = None) -> Union[go.Figure, plt.Figure]:
        """
    ...
    def _plot_trajectories_plotly(self,
                                trajectories: List[List[int]],
                                U: Optional[np.ndarray] = None,
                                M: Optional[np.ndarray] = None,
                                title: str = "Lagrangian Trajectories",
    ...
    def _plot_trajectories_matplotlib(self,
                                    trajectories: List[List[int]],
                                    U: Optional[np.ndarray] = None,
                                    M: Optional[np.ndarray] = None,
                                    title: str = "Lagrangian Trajectories",
        ...
        for i, trajectory in enumerate(trajectories):
            ...
            if traj_x:
                # Plot trajectory path
                ax.plot(traj_x, traj_y, 'o-', color=color, linewidth=3, 
                       markersize=6, label=f'Trajectory {i+1}', alpha=0.8)
                
                # Start and end markers
                ax.plot(traj_x[0], traj_y[0], '*', color='green', markersize=15, 
                       markeredgecolor='black', markeredgewidth=1)
                ax.plot(traj_x[-1], traj_y[-1], 's', color='red', markersize=12,
                       markeredgecolor='black', markeredgewidth=1)
    ...
    def _plot_velocity_field_matplotlib(self,
                                      velocity_field: np.ndarray,
                                      M: Optional[np.ndarray] = None,
                                      title: str = "Network Velocity Field",
                                      time_idx: int = 0,
    ...
    def plot_scheme_comparison(self,
    ...
    def _plot_scheme_comparison_matplotlib(self,
                                         solutions: Dict[str, Tuple[np.ndarray, np.ndarray]],
                                         times: np.ndarray,
        ...
        for node_idx, node in enumerate(selected_nodes):
            ...
            for i, (scheme_name, (U, M)) in enumerate(solutions.items()):
                ax_u.plot(times, U[:, node], label=scheme_name, 
            ...
            for i, (scheme_name, (U, M)) in enumerate(solutions.items()):
                ax_m.plot(times, M[:, node], label=scheme_name, 
                         color=colors[i], linewidth=2)
    ...
    def _add_network_topology_to_matplotlib(self, ax: plt.Axes, node_values: Optional[np.ndarray] = None):
        ...
        if self.node_positions is not None:
            ...
            if node_values is not None:
                scatter = ax.scatter(node_x, node_y, c=node_values, s=100, 
                                   cmap='viridis', alpha=0.8, edgecolors='black')
                plt.colorbar(scatter, ax=ax, label='Node Values')
            else:
                ax.scatter(node_x, node_y, c='lightblue', s=100, 
                          alpha=0.8, edgecolors='black')


# Factory function for enhanced network visualizer
def create_enhanced_network_visualizer(problem: Optional[NetworkMFGProblem] = None,
                                     network_data: Optional[NetworkData] = None) -> EnhancedNetworkMFGVisualizer:
    """
