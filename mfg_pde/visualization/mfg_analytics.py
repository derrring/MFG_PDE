...
class MFGAnalyticsEngine:
    ...
    def analyze_mfg_solution(self, x_grid: np.ndarray, time_grid: np.ndarray,
                           density_history: np.ndarray, value_history: np.ndarray,
                           convergence_data: Optional[List[Dict]] = None,
                           title: str = "MFG Solution Analysis") -> Dict[str, Any]:
        """
    ...
    def analyze_parameter_sweep(self, sweep_results: List[Dict[str, Any]],
                              parameter_name: str = "lambda",
                              title: str = "Parameter Sweep Analysis") -> Dict[str, Any]:
        """
        ...
        if self.sweep_analyzer:
            ...
            if metric_cols:
                effects = self.sweep_analyzer.analyze_parameter_effects(
                    sweep_df, parameter_cols, metric_cols[:3]  # Limit to first 3 metrics
                )
    ...
    def create_research_report(self, analyses: List[Dict[str, Any]], 
                             report_title: str = "MFG Research Report") -> Path:
        """
    ...
    def _analyze_solution_statistics(self, x_grid: np.ndarray, time_grid: np.ndarray,
                                   density_history: np.ndarray, value_history: np.ndarray) -> Dict[str, Any]:
        ...
        # Temporal evolution
        stats["evolution"] = {
            "density_variance_trend": [float(np.var(density_history[i, :])) 
                                     for i in range(0, len(time_grid), max(1, len(time_grid)//10))],
            "value_range_trend": [float(np.max(value_history[i, :]) - np.min(value_history[i, :]))
                                for i in range(0, len(time_grid), max(1, len(time_grid)//10))]
        }
        
        return stats
    
    def _create_solution_visualizations(self, x_grid: np.ndarray, time_grid: np.ndarray,
                                      density_history: np.ndarray, value_history: np.ndarray,
        ...
        density_2d = self.viz_manager.create_2d_density_plot(
            x_grid, time_grid, density_history, "auto",
        ...
        if PLOTLY_AVAILABLE:
            density_3d = self.viz_manager.create_3d_surface_plot(
                x_grid, time_grid, density_history, "density",
            ...
            value_3d = self.viz_manager.create_3d_surface_plot(
                x_grid, time_grid, value_history, "value", 
                f"{title}: 3D Value Function"
            )
    ...
    def _create_convergence_visualizations(self, convergence_data: List[Dict], 
                                         title: str) -> Dict[str, str]:
    ...
    def _export_solution_data(self, x_grid: np.ndarray, time_grid: np.ndarray,
                            density_history: np.ndarray, value_history: np.ndarray,
                            title: str) -> Dict[str, str]:
    ...
    def _generate_html_report(self, analyses: List[Dict[str, Any]], 
...
# Factory function
def create_analytics_engine(prefer_plotly: bool = True, 
                          output_dir: Optional[Union[str, Path]] = None) -> MFGAnalyticsEngine:
    """Create MFG Analytics Engine instance."""
    return MFGAnalyticsEngine(prefer_plotly, output_dir)


# Convenience functions
def analyze_mfg_solution_quick(x_grid: np.ndarray, time_grid: np.ndarray,
                              density_history: np.ndarray, value_history: np.ndarray,
                              title: str = "MFG Analysis") -> Dict[str, Any]:
...
def analyze_parameter_sweep_quick(sweep_results: List[Dict[str, Any]],
                                 parameter_name: str = "lambda",
                                 title: str = "Parameter Sweep") -> Dict[str, Any]:
