...
class PerformanceBaseline:
    ...
    def is_regression(self, metrics: PerformanceMetrics, 
                     time_threshold: float = 1.5, 
                     memory_threshold: float = 1.3) -> Tuple[bool, str]:
        """
...
"""
    ...
    def export_performance_data(self, output_file: Path) -> None:
        ...
        export_data = {
            'baselines': {name: baseline.to_dict() if hasattr(baseline, 'to_dict') 
                         else asdict(baseline) for name, baseline in self.baselines.items()},
            'metrics_history': {name: [m.to_dict() for m in metrics] 
...
def benchmark_solver(solver_class, problem, config_variations: List[Dict[str, Any]], 
