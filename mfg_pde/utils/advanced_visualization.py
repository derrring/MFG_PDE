...
class MFGVisualizer:
    ...
    def _validate_backend(self, backend: str) -> str:
        ...
        if backend == "auto":
            ...
            raise MathematicalVisualizationError(
                "Plotly backend requested but not available"
            )
        elif backend == "matplotlib" and not MATPLOTLIB_AVAILABLE:
            raise MathematicalVisualizationError(
                "Matplotlib backend requested but not available"
            )
    ...
    def plot_mfg_solution(
        ...
        if self.backend == "plotly":
            return self._plot_mfg_solution_plotly(
                U, M, x_grid, t_grid, title, save_path, show
            )
        else:
            return self._plot_mfg_solution_matplotlib(
                U, M, x_grid, t_grid, title, save_path, show
            )
    ...
    def _plot_mfg_solution_matplotlib(
        self, U, M, x_grid, t_grid, title, save_path, show
    ):
    ...
    def plot_convergence_history(
        ...
        """
        self.logger.info(
            f"Creating convergence history plot with {len(convergence_data)} metrics"
        )

        if self.backend == "plotly":
            return self._plot_convergence_plotly(
                convergence_data, title, log_scale, save_path, show
            )
        else:
            return self._plot_convergence_matplotlib(
                convergence_data, title, log_scale, save_path, show
            )

    def _plot_convergence_plotly(
        self, convergence_data, title, log_scale, save_path, show
    ):
    ...
    def _plot_convergence_matplotlib(
        self, convergence_data, title, log_scale, save_path, show
    ):
    ...
    def plot_solution_snapshots(
        ...
        if self.backend == "plotly":
            return self._plot_snapshots_plotly(
                U, M, x_grid, t_indices, t_grid, title, save_path, show
            )
        else:
            return self._plot_snapshots_matplotlib(
                U, M, x_grid, t_indices, t_grid, title, save_path, show
            )

    def _plot_snapshots_plotly(
        self, U, M, x_grid, t_indices, t_grid, title, save_path, show
    ):
    ...
    def _plot_snapshots_matplotlib(
        self, U, M, x_grid, t_indices, t_grid, title, save_path, show
    ):
...
class SolverMonitoringDashboard:
    ...
    def __init__(self, update_interval: float = 1.0):
        ...
        if not PLOTLY_AVAILABLE:
            raise MathematicalVisualizationError(
                "Dashboard requires Plotly. Install with: pip install plotly"
            )
    ...
    def add_performance_data(
        ...
        entry = {
            "duration": duration,
            "timestamp": len(self.performance_data[operation]),
        }
        if metadata:
    ...
    def create_dashboard(
        ...
        fig.update_layout(
            title=dict(
                text="MFG Solver Monitoring Dashboard", x=0.5, font=dict(size=20)
            ),
            height=200 * rows,
            width=1400,
            template="plotly_white",
    ...
    def _add_performance_plots(self, fig):
        ...
        for i, (operation, data) in enumerate(self.performance_data.items()):
            ...
            fig.add_trace(
                go.Bar(
                    x=timestamps, y=durations, name=operation, marker_color="lightblue"
                ),
                row=row,
                col=col,
            )
...
class VisualizationUtils:
    ...
    def _create_plotly_animation(U_sequence, x_grid, title, save_path):
        ...
        fig = go.Figure(
            data=[go.Scatter(x=x_grid, y=U_sequence[0], mode="lines")], frames=frames
        )
    ...
    def _create_matplotlib_animation(U_sequence, x_grid, title, save_path):
        ...
        anim = FuncAnimation(
            fig, animate, frames=len(U_sequence), interval=100, blit=True, repeat=True
        )
    ...
    def save_interactive_report(
        results: Dict[str, Any], output_path: str, title: str = "MFG Analysis Report"
    ) -> str:
        """
