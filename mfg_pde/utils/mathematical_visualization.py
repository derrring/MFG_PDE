...
class MFGMathematicalVisualizer:
    ...
    def __init__(self, backend: str = "auto", enable_latex: bool = True):
        ...
        if enable_latex and MATPLOTLIB_AVAILABLE:
            try:
                ...
                rcParams['font.serif'] = ['Computer Modern Roman', 'DejaVu Serif', 'serif']
        ...
        self.logger.info(f"Mathematical visualizer initialized with backend: {self.backend}")
    ...
    def plot_hjb_analysis(self, 
        ...
        if self.backend == "matplotlib":
            return self._plot_hjb_matplotlib(U, x_grid, t_grid, gradients, title, save_path, show)
        else:
            return self._plot_hjb_plotly(U, x_grid, t_grid, gradients, title, save_path, show)
    
    def _plot_hjb_matplotlib(self, U, x_grid, t_grid, gradients, title, save_path, show):
        ...
        if gradients and 'du_dx' in gradients:
            ...
            contour3 = ax3.contourf(X, T, gradients['du_dx'].T, levels=15, cmap='RdBu_r')
            ax3.set_xlabel(r'$x$', fontsize=14)
            ax3.set_ylabel(r'$t$', fontsize=14)
            ax3.set_title(r'Spatial Gradient $\frac{\partial u}{\partial x}$', fontsize=16)
        ...
        if gradients and 'du_dt' in gradients:
            ...
            contour4 = ax4.contourf(X, T, gradients['du_dt'].T, levels=15, cmap='plasma')
            ax4.set_xlabel(r'$x$', fontsize=14)
            ax4.set_ylabel(r'$t$', fontsize=14)
            ax4.set_title(r'Temporal Gradient $\frac{\partial u}{\partial t}$', fontsize=16)
        ...
        for i, t_idx in enumerate(t_indices):
            ax5.plot(x_grid, U[:, t_idx], color=colors[i], linewidth=2,
    ...
    def plot_fokker_planck_analysis(self,
                                   M: np.ndarray,
                                   x_grid: np.ndarray,
                                   t_grid: np.ndarray,
        ...
        if self.backend == "matplotlib":
            return self._plot_fp_matplotlib(M, x_grid, t_grid, flux, title, save_path, show)
        else:
    ...
    def _plot_fp_matplotlib(self, M, x_grid, t_grid, flux, title, save_path, show):
        ...
        var_x = np.trapz((x_grid[:, np.newaxis] - mean_x[np.newaxis, :])**2 * M, x_grid, axis=0)
        
        ax4.plot(t_grid, mean_x, 'b-', linewidth=2, label=r'$\mathbb{E}[X_t] = \int x m(t,x) dx$')
        ax4_twin = ax4.twinx()
        ax4_twin.plot(t_grid, var_x, 'r-', linewidth=2, label=r'$\mathrm{Var}[X_t] = \int (x-\mu_t)^2 m(t,x) dx$')
        ...
        ax4.legend(lines1 + lines2, labels1 + labels2, bbox_to_anchor=(1.05, 1), loc='upper left', fontsize=9)
        ...
        for i, t_idx in enumerate(t_indices):
            ax5.plot(x_grid, M[:, t_idx], color=colors[i], linewidth=2,
                    label=f'$t = {t_grid[t_idx]:.2f}$')
        ...
        if flux is not None:
            ...
            ax6.set_xlabel(r'$x$', fontsize=14)
            ax6.set_ylabel(r'$t$', fontsize=14)
            ax6.set_title(r'Flux Divergence $-\frac{\partial m}{\partial t}$', fontsize=16)
    ...
    def plot_convergence_theory(self,
    ...
    def _plot_convergence_theory_matplotlib(self, convergence_data, theoretical_rates, title, save_path, show):
        ...
        for i, (metric, values) in enumerate(convergence_data.items()):
            ...
            ax1.semilogy(iterations, values, 'o-', color=color, linewidth=2, 
                        label=metric, markersize=4)
            ...
            if theoretical_rates and metric in theoretical_rates:
                ...
                ax1.semilogy(iterations, theoretical, '--', color=color, alpha=0.7,
        ...
        for i, (metric, values) in enumerate(convergence_data.items()):
            if len(values) > 5:  # Need sufficient data points
                ...
                ax2.plot(log_errors, log_errors_next, 'o', color=colors[i % len(colors)],
                        label=f'{metric}: $\\rho \\approx {estimated_rate:.3f}$')
                ax2.plot(log_errors, predicted, '-', color=colors[i % len(colors)], alpha=0.7)
        
        ax2.set_xlabel(r'$\log(\|e_k\|)$', fontsize=14)
        ax2.set_ylabel(r'$\log(\|e_{k+1}\|)$', fontsize=14)
        ax2.set_title(r'Rate Estimation: $\log(e_{k+1}) \approx \rho \log(e_k)$', fontsize=16)
        ...
        for i, (metric, values) in enumerate(convergence_data.items()):
            if len(values) > 2:
                ...
                ax3.plot(iterations, rates, 'o-', color=colors[i % len(colors)],
                        label=f'{metric}: $e_k/e_{{k-1}}$', linewidth=2, markersize=4)
        ...
        for i, (metric, values) in enumerate(convergence_data.items()):
            if len(values) > 1:
                ...
                ax4.plot(iterations, reductions, 's-', color=colors[i % len(colors)],
                        label=f'{metric}', linewidth=2, markersize=4)
    ...
    def plot_phase_space_analysis(self,
                                 U: np.ndarray,
                                 M: np.ndarray,
                                 x_grid: np.ndarray,
                                 t_grid: np.ndarray,
        ...
        if not MATPLOTLIB_AVAILABLE:
            raise MathematicalVisualizationError("Phase space analysis requires matplotlib")
        ...
        for i, x_idx in enumerate(x_indices[::2]):  # Every other point to avoid clutter
            ...
            for t_idx in range(0, len(t_grid), max(1, len(t_grid)//10)):
                ax1.plot(U_traj[t_idx], M_traj[t_idx], 'o', 
                        color=colors[t_idx], markersize=3)
        ...
        ax2.plot(t_grid, energy, 'b-', linewidth=2, label=r'$E(t) = \int \frac{1}{2}|\nabla u|^2 m \, dx$')
        ...
        ax4.set_xlabel(r'$x$', fontsize=14)
        ax4.set_ylabel(r'$t$', fontsize=14)
        ax4.set_title(r'Optimal Control $\alpha^*(x,t) = -\frac{\partial H}{\partial p}$', fontsize=16)
        ...
        ax5.plot(t_grid, action, 'g-', linewidth=2, label=r'$L(t) = \int m \frac{|\alpha|^2}{2} \, dx$')
        ...
        for i, x_idx in enumerate(range(0, len(x_grid), len(x_grid)//5)):
            ax6.plot(t_grid, costate[x_idx, :], linewidth=2,
...
def quick_hjb_analysis(U: np.ndarray, x_grid: np.ndarray, t_grid: np.ndarray,
...
def quick_fp_analysis(M: np.ndarray, x_grid: np.ndarray, t_grid: np.ndarray,
                     save_path: Optional[str] = None) -> Any:
    """Quick Fokker-Planck analysis with LaTeX mathematical notation."""
    visualizer = MFGMathematicalVisualizer(backend="auto", enable_latex=True)
    return visualizer.plot_fokker_planck_analysis(M, x_grid, t_grid, save_path=save_path)


def quick_phase_space_analysis(U: np.ndarray, M: np.ndarray, 
                              x_grid: np.ndarray, t_grid: np.ndarray,
                              save_path: Optional[str] = None) -> Any:
    """Quick phase space analysis with LaTeX mathematical notation."""
    visualizer = MFGMathematicalVisualizer(backend="auto", enable_latex=True)
    return visualizer.plot_phase_space_analysis(U, M, x_grid, t_grid, save_path=save_path)
...
def demo_mathematical_visualization():
    ...
    try:
        ...
        conv_fig = visualizer.plot_convergence_theory(
            convergence_data,
            theoretical_rates={r'$\|U^k - U^*\|_{L^2}$': 0.8, r'$\|m^k - m^*\|_{L^1}$': 0.85},
