...
class OscillationDetector:
    ...
    def is_stabilized(self, magnitude_threshold: float, 
                     stability_threshold: float) -> Tuple[bool, Dict[str, float]]:
        """
        ...
        if len(self.error_history) < self.history_length:
            return False, {'status': 'insufficient_history', 'samples': len(self.error_history)}
...
class AdvancedConvergenceMonitor:
    ...
    def update(self, u_current: np.ndarray, u_previous: np.ndarray,
               m_current: np.ndarray, x_grid: np.ndarray) -> Dict[str, Any]:
        """
        ...
        if self.iteration_count >= 3:  # Need some history for meaningful assessment
            diagnostics['converged'] = (
                criteria.get('wasserstein', False) and 
                criteria.get('u_stabilized', False)
            )
    ...
    def get_convergence_summary(self) -> Dict[str, Any]:
        ...
        wasserstein_dists = [d.get('wasserstein_distance', np.nan) 
                           for d in self.convergence_history]
        
        # Convergence detection
        converged_iterations = [i for i, d in enumerate(self.convergence_history) 
                              if d['converged']]
    ...
    def plot_convergence_history(self, save_path: Optional[str] = None):
        ...
        ax1.axhline(y=self.u_magnitude_tol, color='r', linestyle='--', 
        ...
        if valid_wasserstein:
            ...
            ax2.axhline(y=self.wasserstein_tol, color='r', linestyle='--',
...
class ParticleMethodDetector:
    ...
    def detect_particle_methods(solver: "MFGSolver") -> Tuple[bool, Dict[str, Any]]:
        ...
        particle_attributes = [
            'num_particles', 'particles', 'particle_solver', 
            'fp_solver', 'particle_fp', 'kde_bandwidth'
        ]
        ...
        particle_class_names = [
            'ParticleFP', 'FPParticle', 'ParticleCollocation', 
        ...
        particle_methods = [
            'update_particles', 'particle_step', 'kde_estimation',
            'particle_density', 'resample_particles'
        ]
        ...
        if hasattr(solver, 'solve'):
            try:
                ...
                particle_params = ['num_particles', 'kde_bandwidth', 'normalize_kde_output']
...
class AdaptiveConvergenceWrapper:
    ...
    def _particle_aware_solve(self, Niter: int = 20, l2errBound: float = None, 
    ...
    def _analyze_solution_convergence(self, U: np.ndarray, M: np.ndarray, 
                                    x_grid: np.ndarray, original_info: Dict) -> Dict[str, Any]:
        """
...
def adaptive_convergence(classical_tol: float = 1e-3,
                        force_particle_mode: Optional[bool] = None,
                        verbose: bool = True,
                        **advanced_kwargs):
    """
