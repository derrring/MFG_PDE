...
from .config import (
    research_config, production_config, fast_config, accurate_config, educational_config,
    crowd_dynamics_config, financial_config, epidemic_config, traffic_config, large_scale_config
)
from .hooks import SolverHooks


def solve_mfg(problem_type: str,
...
def create_mfg_problem(problem_type: str,
                      domain: tuple = (0.0, 1.0),
                      time_horizon: float = 1.0,
                      **kwargs) -> SimpleMFGProblem:
    """
...
def get_config_recommendation(problem_type: str, **kwargs) -> dict[str, Any]:
    ...
    if problem_type == "crowd_dynamics":
        ...
        if crowd_size > 1000:
            recommendation['advice'] = "Large crowd size detected. Consider using domain decomposition for better performance."
        elif crowd_size < 50:
            recommendation['advice'] = "Small crowd size allows for high accuracy. Consider 'accuracy=high' for better results."
...
def solve_mfg_smart(problem_type: str,
                    domain_size: float = 1.0,
                    time_horizon: float = 1.0,
                    validate: bool = True,
                    **kwargs) -> MFGResult:
    ...
    if validate:
        validation = validate_problem_parameters(problem_type,
...
def _create_problem_from_string(problem_type: str, domain_size: float, time_horizon: float, **kwargs) -> SimpleMFGProblem:
...
def solve_mfg_auto(problem_type: str,
                   domain_size: float = 1.0,
                   time_horizon: float = 1.0,
                   verbose: bool = False,
                   **kwargs) -> MFGResult:
