...
class NewtonConfig(BaseModel):
    ...
    tolerance: float = Field(
        ...
        gt=1e-15, 
        le=1e-1, 
        description="Convergence tolerance for Newton method"
    )
    damping_factor: float = Field(
        1.0, 
        gt=0.0, 
        le=1.0, 
        description="Damping parameter for Newton updates"
    )
    line_search: bool = Field(
        False, 
        description="Whether to use line search for step size control"
    )
    verbose: bool = Field(
        False, 
        description="Whether to print Newton iteration details"
    )
...
class PicardConfig(BaseModel):
    ...
    tolerance: float = Field(
        ...
        gt=1e-12, 
        le=1.0, 
        description="Convergence tolerance for Picard method"
    )
    damping_factor: float = Field(
        0.5, 
        gt=0.0, 
        le=1.0, 
        description="Damping parameter for Picard updates"
    )
    adaptive_damping: bool = Field(
        False, 
        description="Whether to use adaptive damping"
    )
    verbose: bool = Field(
        False, 
        description="Whether to print Picard iteration details"
    )
...
class GFDMConfig(BaseModel):
    ...
    delta: float = Field(
        ...
        gt=0.0, 
        le=1.0, 
        description="GFDM delta parameter"
    )
    weight_function: str = Field(
        "gaussian", 
        description="Weight function type"
    )
    use_qp_constraints: bool = Field(
        True, 
        description="Whether to use QP constraints"
    )
    constraint_tolerance: float = Field(
        1e-8, 
        gt=0.0, 
        le=1e-3, 
        description="QP constraint tolerance"
    )
    max_neighbors: int = Field(
        10, 
        ge=3, 
        le=50, 
        description="Maximum number of neighbors for GFDM"
    )
    ...
    def validate_delta_stability(cls, v):
        ...
        if v > 0.5:
            warnings.warn(
                f"Large delta ({v:.3f}) may reduce accuracy",
                UserWarning
            )
        return v
...
class ParticleConfig(BaseModel):
    ...
    boundary_treatment: str = Field(
        "reflection", 
        description="Boundary treatment method"
    )
    resampling_method: str = Field(
        "systematic", 
        description="Particle resampling method"
    )
    adaptive_particles: bool = Field(
        False, 
        description="Whether to use adaptive particle count"
    )
    ...
    def validate_particle_count(cls, v):
        ...
        if v < 1000:
            warnings.warn(
                f"Few particles ({v}) may give poor density approximation",
                UserWarning
            )
        if v > 100000:
            warnings.warn(
                f"Many particles ({v}) may be computationally expensive",
                UserWarning
            )
        return v
...
class FPConfig(BaseModel):
    ...
    particle: ParticleConfig = Field(
        default_factory=ParticleConfig, 
        description="Particle method configuration"
    )
    solver_type: str = Field(
        "particle", 
        description="FP solver type"
    )
    time_integration: str = Field(
        "euler", 
...
class MFGSolverConfig(BaseModel):
    ...
    picard: PicardConfig = Field(
        default_factory=PicardConfig, 
        description="Picard iteration configuration"
    )
    hjb: HJBConfig = Field(
        default_factory=HJBConfig, 
        description="HJB solver configuration"
    )
    fp: FPConfig = Field(
        default_factory=FPConfig, 
    ...
    enable_warm_start: bool = Field(
        False, 
        description="Whether to enable warm start capability"
    )
