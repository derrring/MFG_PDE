...
def compute_hjb_residual(U_n_current_guess, U_n_plus_1_known, M_density_for_H, problem):
    """
    ...
    for i in range(Nx):
        ...
        p1_U_n = (
            U_n_current_guess[ip1] - U_n_current_guess[i]
        ) / Dx  # Forward difference
        p2_U_n = (
            U_n_current_guess[i] - U_n_current_guess[im1]
        ) / Dx  # Backward difference
...
def compute_hjb_jacobian(U_n_current_guess, problem):
    ...
    A_L[
        1:
    ] += A_L_val  # For U_n_current_guess[i-1] component (A_L[0] is for Jac[0,-1] if periodic)
    A_U[
        :-1
    ] += A_U_val  # For U_n_current_guess[i+1] component (A_U[Nx-1] is for Jac[Nx-1,Nx] if periodic)
    ...
    A_D[1 : Nx - 1] += (
        coefCT
        * (
            problem._npart(U_for_H_jac[2:Nx] - U_for_H_jac[1 : Nx - 1])
            + problem._ppart(U_for_H_jac[1 : Nx - 1] - U_for_H_jac[0 : Nx - 2])
        )
        / (Dx**2)
    )
    A_D[Nx - 1] += (
        coefCT
        * (
            problem._npart(U_for_H_jac[0] - U_for_H_jac[Nx - 1])
            + problem._ppart(U_for_H_jac[Nx - 1] - U_for_H_jac[Nx - 2])
        )
        / (Dx**2)
    )  # Periodic
    A_D[0] += (
        coefCT
        * (
            problem._npart(U_for_H_jac[1] - U_for_H_jac[0])
            + problem._ppart(U_for_H_jac[0] - U_for_H_jac[Nx - 1])
        )
        / (Dx**2)
    )  # Periodic
    ...
    A_L[1:Nx] += (
        -coefCT * problem._ppart(U_for_H_jac[1:Nx] - U_for_H_jac[0 : Nx - 1]) / Dx**2
    )
    # dH/d(U[i+1]) terms add to A_U[i]
    A_U[0 : Nx - 1] += (
        coefCT
        * (-problem._npart(U_for_H_jac[1:Nx] - U_for_H_jac[0 : Nx - 1]))
        / (Dx**2)
    )
...
def newton_hjb_step(U_n_current_guess, U_n_plus_1_known, M_density_for_H, problem):
    """Performs one step of Newton's method for the HJB equation at a time slice."""
    Dx = problem.Dx

    residual_F_U = compute_hjb_residual(
        U_n_current_guess, U_n_plus_1_known, M_density_for_H, problem
    )
    ...
    l2_error_of_step = np.linalg.norm(delta_U) * np.sqrt(
        Dx
    )  # Error based on update step size

    return U_n_next_iteration, l2_error_of_step


def solve_hjb_timestep_newton(
    U_n_plus_1_known, M_density_for_H, problem, NiterNewton, l2errBoundNewton
):
    ...
    for iiter in range(NiterNewton):
        U_n_next_iteration, l2_error = newton_hjb_step(
            U_n_current_iteration, U_n_plus_1_known, M_density_for_H, problem
        )
    ...
    if not converged:
        print(
            f"Warning: Newton method for HJB did not converge at this time slice. Error: {l2_error:.2e} (Bound: {l2errBoundNewton:.2e})"
        )
...
def solve_hjb_system_backward(
    ...
    for n in range(Nt - 1, -1, -1):
        ...
        U_solution[n] = solve_hjb_timestep_newton(
            U_n_plus_1, M_for_H_at_this_step, problem, NiterNewton, l2errBoundNewton
        )
    return U_solution
