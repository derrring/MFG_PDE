...
class HJBGFDMOptimizedSolver(HJBGFDMSolver):
    ...
    def _check_derivative_monotonicity_violation(self, solution: np.ndarray, 
                                               point_idx: int, taylor_data: Dict) -> bool:
    ...
    def _solve_unconstrained_hjb(self, taylor_data: Dict, b: np.ndarray, 
    ...
    def _solve_qp_cvxpy(self, taylor_data: Dict, b: np.ndarray, point_idx: int,
    ...
    def _solve_qp_scipy_fallback(self, taylor_data: Dict, b: np.ndarray, 
                               point_idx: int) -> Tuple[np.ndarray, bool]:
    ...
    def _solve_batch_qp(self, taylor_data_batch: List[Dict], b_batch: List[np.ndarray],
                       point_indices: List[int]) -> List[Tuple[np.ndarray, bool]]:
        ...
        try:
            ...
            var_counts = [td.get('taylor_matrix', np.eye(len(b))).shape[1] 
            ...
            for i, (taylor_data, b, point_idx, n_vars) in enumerate(zip(
                taylor_data_batch, b_batch, point_indices, var_counts)):
            ...
            if OSQP_AVAILABLE:
                batch_problem.solve(solver=cp.OSQP, verbose=False, 
                                   eps_abs=1e-4, eps_rel=1e-4, max_iter=1000)
            else:
    ...
    def solve_collocation_point_optimized(self, point_idx: int, taylor_data: Dict, 
                                        b: np.ndarray) -> Tuple[np.ndarray, float, bool]:
        """
    ...
    def solve_hjb_system(self, U_current: np.ndarray, M_current: np.ndarray, 
                        n_time_idx: int) -> np.ndarray:
        """
                ...
                if len(qp_indices) == 0:
                    ...
                    for i, (point_idx, solution) in enumerate(zip(point_indices, unconstrained_solutions)):
                        if i in qp_indices:
                            ...
                            U_new[point_idx, :] = qp_solution[:U_new.shape[1]] if len(qp_solution) >= U_new.shape[1] else qp_solution
                        else:
    ...
    def _build_taylor_data(self, point_idx: int, U_current: np.ndarray, 
                          M_current: np.ndarray, n_time_idx: int) -> Dict:
    ...
    def _build_rhs_vector(self, point_idx: int, U_current: np.ndarray, 
                         M_current: np.ndarray, n_time_idx: int) -> np.ndarray:
    ...
    def print_performance_summary(self):
        ...
        print(f"  QP Calls Skipped: {stats['qp_calls_skipped']}/{stats['total_qp_calls']} ({stats['qp_skip_rate']:.1%})")
