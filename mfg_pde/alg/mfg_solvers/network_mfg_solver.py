...
class NetworkFixedPointIterator(MFGSolver):
    ...
    def __init__(self,
        ...
        self.fp_solver = create_network_fp_solver(
            problem, fp_solver_type, **fp_kwargs
        )
...
class NetworkFlowMFGSolver(NetworkFixedPointIterator):
    ...
    def analyze_network_flows(self) -> Dict[str, Any]:
        ...
        avg_flows = np.mean([self._compute_flows_at_time(self.U[t, :], self.M[t, :]) 
                           for t in range(self.Nt + 1)], axis=0)
...
def create_network_mfg_solver(problem: "NetworkMFGProblem",
                             solver_type: str = "fixed_point",
                             **kwargs) -> NetworkFixedPointIterator:
    """
