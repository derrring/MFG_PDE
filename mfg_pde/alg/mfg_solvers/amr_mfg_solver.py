...
class AMRMFGSolver(MFGSolver):
    ...
    def _compute_fp_update(self, U: NDArray, M: NDArray, dx: float) -> NDArray:
        ...
        laplacian_M = (np.roll(M, 1, axis=0) + np.roll(M, -1, axis=0) + 
    ...
    def _compute_solution_change(
        self, 
        U_old: NDArray, M_old: NDArray, 
        U_new: NDArray, M_new: NDArray
    ) -> float:
    ...
    def _get_region_values(
        ...
        if len(i_indices) == 0 or len(j_indices) == 0:
            ...
            i_center = int((node.center_x - self.adaptive_mesh.x_min) / 
                          (self.adaptive_mesh.x_max - self.adaptive_mesh.x_min) * U.shape[0])
            j_center = int((node.center_y - self.adaptive_mesh.y_min) / 
                          (self.adaptive_mesh.y_max - self.adaptive_mesh.y_min) * U.shape[1])
        ...
        return {
            ...
            'efficiency_metrics': {
                ...
                'average_efficiency': (
                    ...
                    if self.amr_stats['mesh_efficiency'] else 0.0
                )
            }
        }
...
class JAXAcceleratedAMR:
    ...
    def conservative_interpolation_1d(
        ...
        coarse_coords: jnp.ndarray,
        fine_coords: jnp.ndarray
    ) -> jnp.ndarray:
        """
    ...
    **kwargs
    ...
    if domain_bounds is None:
        domain_bounds = (
            ...
            problem.xmin, problem.xmax  # Assume square domain for now
        )
