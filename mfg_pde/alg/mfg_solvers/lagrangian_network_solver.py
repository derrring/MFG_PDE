...
class LagrangianNetworkMFGSolver(NetworkFixedPointIterator):
    ...
    def _setup_velocity_grid(self) -> np.ndarray:
        ...
        if self.velocity_dim == 1:
            ...
            return np.random.uniform(-2.0, 2.0, 
                                   (self.velocity_discretization, self.velocity_dim))
    ...
    def _solve_lagrangian_fixed_point(self, max_iterations: int, tolerance: float, 
                                    verbose: bool, **kwargs) -> Tuple[np.ndarray, np.ndarray, Dict[str, Any]]:
    ...
    def _compute_future_cost(self, node: int, velocity: np.ndarray, 
                            u_next: np.ndarray, t: float) -> float:
    ...
    def _get_edge_direction(self, node_from: int, node_to: int) -> np.ndarray:
        ...
        if (self.network_problem.network_data.node_positions is not None and
            self.velocity_dim >= 2):
    ...
    def _solve_relaxed_equilibria(self, max_iterations: int, tolerance: float,
                                verbose: bool, **kwargs) -> Tuple[np.ndarray, np.ndarray, Dict[str, Any]]:
        ...
        self.trajectory_measures = [self._create_initial_trajectory_measure() 
...
def create_lagrangian_network_solver(problem: "NetworkMFGProblem",
                                    solver_type: str = "lagrangian",
                                    **kwargs) -> LagrangianNetworkMFGSolver:
    """
