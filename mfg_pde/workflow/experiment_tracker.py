        ...
        try:
            ...
            if metadata_file.exists():
                ...
                self.metadata = ExperimentMetadata(
                    ...
                    started_time=datetime.fromisoformat(metadata_dict['started_time']) if metadata_dict.get('started_time') else None,
                    completed_time=datetime.fromisoformat(metadata_dict['completed_time']) if metadata_dict.get('completed_time') else None,
            ...
            if this_result is not None and other_result is not None:
                ...
                if isinstance(this_result, (int, float)) and isinstance(other_result, (int, float)):
                    comparison['result_differences'][result_name] = {
                        ...
                        'relative_difference': abs(this_result - other_result) / max(abs(this_result), abs(other_result), 1e-10)
                    }
                # Array comparison
                    ...
                    if this_result.shape == other_result.shape:
                        comparison['result_differences'][result_name] = {
                            ...
                            'relative_error': np.linalg.norm(this_result - other_result) / max(np.linalg.norm(this_result), np.linalg.norm(other_result), 1e-10)
                        }
...
class ExperimentTracker:
    ...
    def create_experiment(self, name: str, description: str = "", 
                         tags: Optional[List[str]] = None,
                         workflow: Optional['Workflow'] = None) -> Experiment:
