...
def workflow_step(workflow: Optional[Workflow] = None, 
...
def parameter_study(parameters: Dict[str, List[Any]],
...
def cached(cache_dir: Optional[str] = None, 
            ...
            for attempt in range(max_attempts):
                try:
                    ...
                    if attempt < max_attempts - 1:
                        print(f"⚠️ {func.__name__} failed on attempt {attempt + 1}, retrying in {delay:.1f}s...")
                        time.sleep(delay)
...
def mfg_solver(problem_type: str = "general",
    ...
    def decorator(func: Callable) -> Callable:
        ...
        if log_performance:
            ...
            decorated_func = log_execution(
                logger_name=f"mfg_solver.{problem_type}",
        ...
        if cache_results:
            decorated_func = cached(
                cache_dir=f".mfg_cache/{problem_type}",
...
def convergence_study(tolerances: List[float],
