...
Migration Note (Issue #628):
    This module was moved from `mfgarchon.solvers.fixed_point` to consolidate
    all solver code under `mfgarchon.alg/`. The original location remains
...
from mfgarchon.types import ConvergenceInfo, MFGProblem, SolutionArray, SpatialTemporalState
...
class FixedPointSolver(BaseIterativeSolver):
    ...
    Example with hooks:
        from mfgarchon.hooks import DebugHook
    ...
    def _initialize_state(self, problem: MFGProblem) -> SpatialTemporalState:
        ...
        try:
            ...
            if m_init.shape != (Nt + 1, Nx + 1):
                ...
                # Normalize
                m_init[0, :] /= float(np.trapezoid(m_init[0, :], x_grid))
    ...
    def _solve_fp_step(
        ...
        for n in range(len(t_grid) - 1):
            ...
            total_mass = float(np.trapezoid(m_new[n, :], x_grid))
            if total_mass > 0:
                m_new[n, :] /= total_mass
