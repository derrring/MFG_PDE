...
from mfg_pde.utils.exceptions import ConvergenceError, ConfigurationError
...
class TestMFGMathematicalProperties:
    ...
    def test_mass_conservation_property(self, Nx, Nt, sigma, coupling_coefficient, T):
        ...
        try:
            problem = ExampleMFGProblem(
                Nx=Nx, Nt=Nt, T=T, 
                sigma=sigma, coefCT=coupling_coefficient
            )
            ...
            assert relative_mass_change < 0.05, \
                f"Mass not conserved: {relative_mass_change:.6f} > 0.05"
    ...
    @given(
        Nx=spatial_points,
        sigma1=diffusion_coeff,
        sigma2=diffusion_coeff
    )
    ...
    def test_monotonicity_in_diffusion_property(self, Nx, sigma1, sigma2):
        ...
        try:
            ...
            assert relative_smoothing > -0.2, \
                f"Diffusion monotonicity violated: {relative_smoothing:.3f}"
...
class TestMFGNumericalStability:
    ...
    @given(
        base_Nx=st.integers(min_value=15, max_value=25),
        refinement_factor=st.integers(min_value=2, max_value=3)
    )
...
class TestMFGParameterValidation:
    ...
    @given(
        Nx=st.integers(min_value=1, max_value=5),  # Too small grids
            ...
            if tolerance <= 0:
                ...
                if tolerance < 1e-10:
                    ...
                    if info.get("converged", False):
                        ...
                        assert final_error <= tolerance * 10, \
