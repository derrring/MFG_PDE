...
def test_kde_normalization():
    ...
    problem = ExampleMFGProblem(
        xmin=0.0, xmax=1.0, Nx=20, T=0.1, Nt=5, sigma=1.0, coefCT=0.5
    )
    ...
    M_result1 = solver1.solve_fp_system(
        m_initial_condition=problem.m_init,
    ...
    M_result2 = solver2.solve_fp_system(
        m_initial_condition=problem.m_init,
        U_solution_for_drift=U_zero
    )
    ...
    M_result3 = solver3.solve_fp_system(
        m_initial_condition=problem.m_init,
        U_solution_for_drift=U_zero
    )
