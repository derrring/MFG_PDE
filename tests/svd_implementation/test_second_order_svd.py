...
def test_second_order_svd():
    ...
    problem = ExampleMFGProblem(xmin=0.0, xmax=1.0, Nx=20, T=0.1, Nt=10, sigma=0.5, coefCT=0.1)
    ...
    for delta in delta_values:
        ...
        try:
            ...
            if decomp_info["condition_numbers"]:
                print(
                    f"  Condition number range: [{min(decomp_info['condition_numbers']):.1e}, {max(decomp_info['condition_numbers']):.1e}]"
                )
            ...
            if M is not None:
                ...
                print("Testing derivative accuracy:")
