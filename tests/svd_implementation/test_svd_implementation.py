...
def test_svd_implementation():
    ...
    if decomp_info['condition_numbers']:
        ...
        print(f"Condition number range: [{min(decomp_info['condition_numbers']):.2e}, {max(decomp_info['condition_numbers']):.2e}]")
    ...
    try:
        ...
        if (1,) in derivatives:
            ...
            print(f"First derivative: numerical={numerical_first:.6f}, analytical={analytical_first:.6f}, error={error_first:.2e}")
        
        if (2,) in derivatives:
            numerical_second = derivatives[(2,)]
            error_second = abs(numerical_second - analytical_second)
            print(f"Second derivative: numerical={numerical_second:.6f}, analytical={analytical_second:.6f}, error={error_second:.2e}")
