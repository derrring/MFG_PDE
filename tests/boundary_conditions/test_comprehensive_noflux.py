...
def test_comprehensive_noflux():
    ...
    for i, case in enumerate(test_cases):
        ...
        problem = ExampleMFGProblem(
            xmin=0.0, xmax=1.0, 
            Nx=case['Nx'], T=case['T'], Nt=case['Nt'], 
        ...
        try:
            ...
            for b_idx in [0, num_collocation_points-1]:
                ...
                boundary_info.append({
                    'index': b_idx,
                    'ghosts': neighborhood['ghost_count'],
                    'total_neighbors': neighborhood['size']
                })
            
            print(f"SVD: {decomp_info['svd_percentage']:.0f}%, κ={decomp_info['avg_condition_number']:.1e}")
            print(f"Ghosts: [{boundary_info[0]['ghosts']}, {boundary_info[1]['ghosts']}], Neighbors: [{boundary_info[0]['total_neighbors']}, {boundary_info[1]['total_neighbors']}]")
