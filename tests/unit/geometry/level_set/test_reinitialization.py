...
class TestReinitializationGlobal:
    ...
    def test_circle_2d(self):
        ...
        grid = TensorProductGrid(
            dimension=2, bounds=[(0.0, 1.0), (0.0, 1.0)], Nx=[50, 50], boundary_conditions=no_flux_bc(dimension=2)
        )
        X, Y = grid.meshgrid()
...
class TestReinitializationNarrowBand:
    ...
    def test_narrow_band_correctness_2d(self):
        ...
        grid = TensorProductGrid(
            dimension=2, bounds=[(0.0, 1.0), (0.0, 1.0)], Nx=[100, 100], boundary_conditions=no_flux_bc(dimension=2)
        )
        X, Y = grid.meshgrid()
        dx = grid.spacing[0]
