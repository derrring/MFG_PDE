...
import pytest
import numpy as np
from mfg_pde import ExampleMFGProblem, create_fast_solver, create_accurate_solver
...
class TestMassConservation:
    ...
    def test_non_negativity_property(self, small_problem):
        ...
        assert min_density >= -1e-10, (
            f"Negative density found: min={min_density:.6e}"
        )
    ...
    def test_mass_conservation_problem_scaling(self, problem_size):
        ...
        assert relative_error < 0.02, (  # 2% relative error
            f"Mass conservation poor for problem size {problem_size}: "
    ...
    def test_mass_conservation_time_evolution(self, small_problem):
        ...
        for t_idx in range(small_problem.Nt + 1):
            ...
            assert mass_error < 1e-2, (
                f"Mass not conserved at time step {t_idx}: error={mass_error:.6e}"
            )
        ...
        if len(mass_errors) > 3:
            ...
            is_consistently_increasing = (
                last_three[1] > last_three[0] and 
                last_three[2] > last_three[1]
            )
            assert not is_consistently_increasing, (
                "Mass conservation errors are consistently increasing, "
...
class TestPhysicalProperties:
    ...
    def test_probability_density_normalization(self, small_problem):
        ...
        for t_idx in range(small_problem.Nt + 1):
            ...
            assert abs(total_probability - 1.0) < 0.02, (
                f"Density not normalized at t_idx={t_idx}: "
                f"integral={total_probability:.6f}"
            )
    ...
    def test_energy_bounds(self, small_problem):
        ...
        assert max_abs_value < 1000, (
            f"Value function has extreme values: max_abs={max_abs_value:.2f}"
        )
    ...
    def test_initial_condition_preservation(self, small_problem):
        ...
        assert max_diff < 1e-10, (
            f"Initial condition not preserved: max_diff={max_diff:.6e}"
        )
