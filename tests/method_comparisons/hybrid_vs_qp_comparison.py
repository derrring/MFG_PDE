...
def compare_hybrid_vs_qp():
    ...
    try:
        ...
        U_hybrid, M_hybrid, iterations_hybrid, l2dist_u_h, l2dist_m_h = hybrid_solver.solve(
            ...
            l2errBoundPicard=shared_settings["picard_tolerance"]
        )
    ...
    try:
        ...
        print(f"  Starting QP solve with {shared_settings['num_particles']} particles, {num_collocation_points} collocation points...")
        U_qp, M_qp, info_qp = qp_solver.solve(
            Niter=shared_settings["picard_iterations"],
            l2errBound=shared_settings["picard_tolerance"],
            verbose=False
        )
...
def analyze_hybrid_qp_comparison(results, problem):
    ...
    for metric_name, category, key, fmt in metrics:
        ...
        if isinstance(hybrid_val, (int, float)) and isinstance(qp_val, (int, float)):
            ...
            diff_str = f"{abs(int(qp_val) - int(hybrid_val))}" if str(hybrid_val).isdigit() and str(qp_val).isdigit() else "N/A"
...
def create_hybrid_qp_plots(results, problem):
    ...
    for method in successful_methods:
        ...
        ax3.plot(problem.tSpace, mass_evolution, 'o-', 
    ...
    if len(successful_methods) == 2:
        ...
        bars1 = ax4.bar(x_pos - width/2, hybrid_vals, width, 
                        label='Hybrid', color='green', alpha=0.8)
        bars2 = ax4.bar(x_pos + width/2, qp_vals, width, 
                        label='QP-Collocation', color='red', alpha=0.8)
    ...
    if len(successful_methods) == 2:
        ...
        bars1 = ax5.bar(x_pos - width/2, times, width, 
                        label='Time (s)', color='blue', alpha=0.8)
        
        # Secondary y-axis for iterations
        ax5_twin = ax5.twinx()
        bars2 = ax5_twin.bar(x_pos + width/2, iterations, width, 
    ...
    plt.tight_layout()
    plt.savefig('/Users/zvezda/Library/CloudStorage/OneDrive-Personal/code/MFG_PDE/hybrid_vs_qp_comparison.png', 
