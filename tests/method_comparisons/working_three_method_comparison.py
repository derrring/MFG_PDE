        ...
        for picard_iter in range(Niter):
            ...
            for t_idx in range(Nt - 1, -1, -1):
                for i in range(1, Nx):
                    ...
                    hamiltonian = min(abs(hamiltonian), 1.0) * np.sign(hamiltonian) if not np.isnan(hamiltonian) else 0.0
                    ...
                    update = -dt_factor * self.problem.Dt * (hamiltonian + coupling - 0.5 * self.problem.sigma ** 2 * U_xx)
...
class SimpleHybridSolver(MFGSolver):
        ...
        for picard_iter in range(Niter):
            ...
            for t_idx in range(Nt - 1, -1, -1):
                for i in range(1, Nx):
                    ...
                    U[t_idx, i] = U[t_idx + 1, i] - self.problem.Dt * (hamiltonian + coupling - 0.5 * self.problem.sigma ** 2 * U_xx)
    ...
    def _estimate_density_from_particles(self, particles):
        ...
        for t_idx in range(Nt):
            hist, _ = np.histogram(particles[t_idx, :], bins=Nx-1, 
...
def compare_three_methods():
    ...
    try:
        ...
        if M_hybrid is not None and U_hybrid is not None:
            ...
            print(f"✓ Hybrid completed: mass_change={results['hybrid']['mass_change']:.2e}, time={time_hybrid:.2f}s, violations={violations}")
        else:
    ...
    try:
        ...
        U_colloc, M_colloc, info_colloc = collocation_solver.solve(
            Niter=10, l2errBound=1e-3, verbose=False
        )
        ...
        if M_colloc is not None and U_colloc is not None:
            ...
            print(f"✓ QP-Collocation completed: mass_change={results['collocation']['mass_change']:.2e}, time={time_colloc:.2f}s, violations={violations}")
        else:
