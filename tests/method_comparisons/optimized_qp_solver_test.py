...
class OptimizedQPSolverTester:
    ...
    def create_baseline_solver(self, problem, collocation_points, boundary_indices, 
                              boundary_conditions, scenario):
    ...
    def create_optimized_solver(self, problem, collocation_points, boundary_indices,
                               boundary_conditions, scenario):
    ...
    def run_solver_test(self, solver, scenario_name, solver_type):
        ...
        try:
            U, M, solve_info = solver.solve(
                Niter=scenario['max_iterations'],
                l2errBound=scenario['convergence_tolerance'],
                verbose=False
            )
            ...
            if M is not None and len(M) > 0:
                initial_mass = np.sum(M[0, :]) * self.problem_params.get('Dx', 1.0/(self.problem_params.get('Nx', 25)))
    ...
    def run_comparative_test(self, scenario_name):
        ...
        try:
            baseline_solver = self.create_baseline_solver(
                problem, collocation_points, boundary_indices, 
                boundary_conditions, scenario
            )
        ...
        if baseline_result['success'] and optimized_result['success']:
            ...
            if (baseline_result.get('U') is not None and 
                optimized_result.get('U') is not None):
    ...
    def print_summary(self):
        ...
        for scenario_name, result in self.results.items():
            ...
            overall_status = "PASS" if (baseline['success'] and optimized['success'] and speedup and speedup > 1.0) else "FAIL"
            
            print("{:<20} {:<15} {:<15} {:<12} {:<12} {:<10}".format(
                scenario_name[:19], 
    ...
    def create_performance_plots(self):
        ...
        valid_indices = [i for i, (b, o) in enumerate(zip(baseline_times, optimized_times)) 
        ...
        if valid_speedups:
            ...
            ax2.axhline(y=target_speedup, color='red', linestyle='--', 
        ...
        if baseline_mass_errors and optimized_mass_errors:
            ...
            ax3.plot([0, max(max(baseline_mass_errors), max(optimized_mass_errors))],
        ...
        if qp_activation_rates:
            ax4.bar(range(len(qp_activation_rates)), qp_activation_rates, 
            ...
            ax4.axhline(y=expected_rate, color='red', linestyle='--',
                       label=f'Expected: {expected_rate}%')
            ax4.legend()
        
        plt.tight_layout()
        plt.savefig('/Users/zvezda/Library/CloudStorage/OneDrive-Personal/code/MFG_PDE/tests/method_comparisons/optimized_qp_solver_performance.png', 
