...
def test_qp_collocation_optimized(problem_params, qp_params, test_name):
    ...
    try:
        ...
        U_qp, M_qp, solve_info = qp_solver.solve(
            Niter=qp_params['max_iterations'],
            l2errBound=qp_params['convergence_tolerance'],
            verbose=True
        )
...
def run_qp_robustness_study():
    ...
    base_problem = {
        'xmin': 0.0, 'xmax': 1.0, 'Nx': 30, 'T': 1.0, 'Nt': 50, 
    ...
    for config in test_configs:
        ...
        result = test_qp_collocation_optimized(
            base_problem, 
            config['qp_params'], 
            config['name']
        )
...
def analyze_qp_robustness_results(results):
    ...
    if len(successful_results) >= 2:
        ...
        if standard and optimized:
            ...
            print(f"Optimized vs Standard: {time_improvement:+.1f}% time change, {mass_improvement:+.3f}pp mass improvement")
        
        if standard and no_constraints:
            constraint_overhead = (standard['solve_time'] - no_constraints['solve_time']) / no_constraints['solve_time'] * 100
...
def create_qp_robustness_plots(results):
    ...
    for i, result in enumerate(successful_results):
        if 'mass_evolution' in result and 'problem' in result:
            ax3.plot(result['problem'].tSpace, result['mass_evolution'], 
                    color=colors[i], linewidth=2, label=result['test_name'].replace('_', ' '))
    ...
    for i in range(len(configs)):
        for j in range(len(metrics)):
            ax4.text(j, i, f'{config_scores[i][j]:.2f}', ha='center', va='center', 
                    color='white' if config_scores[i][j] < 0.5 else 'black', fontweight='bold')
    
    # Add colorbar
    plt.colorbar(im, ax=ax4, label='Performance Score (0-1)')
    
    plt.tight_layout()
    plt.savefig('/Users/zvezda/Library/CloudStorage/OneDrive-Personal/code/MFG_PDE/tests/method_comparisons/qp_robustness_study.png', 
