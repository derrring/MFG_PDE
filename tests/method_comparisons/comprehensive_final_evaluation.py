...
def comprehensive_three_method_evaluation():
    ...
    problem_params = {
        'xmin': 0.0, 'xmax': 1.0, 'Nx': 20, 'T': 1.0, 'Nt': 30,
    ...
    try:
        ...
        fdm_hjb_solver = FdmHJBSolver(
            problem=problem
        )
        
        # Create FDM FP solver
        fdm_fp_solver = FdmFPSolver(
            problem=problem,
    ...
    try:
        ...
        # Create FDM HJB solver
        hybrid_hjb_solver = FdmHJBSolver(
            problem=problem
        )
        ...
        hybrid_fp_solver = ParticleFPSolver(
            problem=problem,
            num_particles=num_particles_hybrid,
            boundary_conditions=no_flux_bc
        )
...
def print_comprehensive_comparison(results):
    ...
    for key, result in results.items():
        ...
        print(f"{result['method']:<25} {success_str:<8} {time_str:<10} {mass_str:<12} {converged_str:<10} {iter_str:<11} {char_summary:<30}")
...
def create_comprehensive_plots(results, problem_params):
    ...
    ax3.text(0.05, 0.95, char_text, transform=ax3.transAxes, fontsize=10,
    ...
    for i, (key, result) in enumerate(successful_results.items()):
        if i < 3:  # Only plot first 3 methods
            ...
            for t_idx in [0, M.shape[0]//2, -1]:
                ...
                ax.plot(x_grid, M[t_idx, :], label=f'{label} Density', 
                       color=colors[i], alpha=alpha, linewidth=2 if alpha == 1.0 else 1)
    ...
    for i, (key, result) in enumerate(successful_results.items()):
        ax7.scatter(result['time'], result['mass_error'], 
                   color=colors[i], s=150, alpha=0.7, label=result['method'])
        ax7.annotate(result['method'], 
    ...
    if 'qp_collocation' in successful_results:
        ...
        ax8.text(0, -1.3, f'Quality: {quality}', ha='center', 
                bbox=dict(boxstyle="round,pad=0.3", facecolor="yellow", alpha=0.7))
    else:
        ax8.text(0.5, 0.5, 'QP-Collocation\nNot Available', 
    ...
    ax9.text(0.05, 0.95, recommendations, transform=ax9.transAxes, fontsize=11,
