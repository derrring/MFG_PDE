...
class ComprehensiveThreeMethodEvaluator:
        ...
        for method_name, create_method in methods:
            ...
            try:
                ...
                if success and M is not None:
                    # Mass conservation
                    Dx = problem_params.get('Dx') or (problem_params['xmax'] - problem_params['xmin']) / problem_params['Nx']
    ...
    def run_comprehensive_evaluation(self, test_scenarios, max_time_minutes=30):
        ...
        for scenario_name, scenario_config in test_scenarios.items():
            ...
            scenario_results = self.run_single_test(
                scenario_name, problem_params, solver_params, max_time_minutes
            )
    ...
    def print_summary(self):
        ...
        for scenario_name, scenario_results in self.results.items():
            for method_name, result in scenario_results.items():
                ...
                mass_err_str = f"{result['mass_error_percent']:.1f}" if result['mass_error_percent'] != float('inf') else "∞"
                ...
                print(f"{scenario_name:<20} {result['method_name']:<20} {success_str:<8} {result['solve_time_minutes']:<10.1f} {mass_err_str:<12} {converged_str:<10} {status:<15}")
        ...
        for method in methods:
            ...
            if method_results:
                ...
                mass_errors = [r['mass_error_percent'] for r in method_results 
        ...
        if method_stats:
            ...
            best_speed = min(method_stats.items(), key=lambda x: x[1]['avg_time'] if not np.isnan(x[1]['avg_time']) else float('inf'))
            best_mass = min(method_stats.items(), key=lambda x: x[1]['avg_mass_error'] if not np.isinf(x[1]['avg_mass_error']) else float('inf'))
    ...
    def create_comparison_plots(self):
        ...
        for i, method in enumerate(methods):
            ...
            ax1.bar(x_pos + i*width, times, width, label=method_labels[i], 
        ...
        for i, method in enumerate(methods):
            mass_errors = []
            for scenario in scenarios:
                if (method in self.results[scenario] and 
                    self.results[scenario][method]['success'] and
                    self.results[scenario][method]['mass_error_percent'] != float('inf')):
                    mass_errors.append(self.results[scenario][method]['mass_error_percent'])
                else:
                    mass_errors.append(100)  # High error for failed cases
            
            ax2.bar(x_pos + i*width, mass_errors, width, label=method_labels[i], 
                   color=colors[i], alpha=0.7)
        ...
        if qp_scenarios:
            ...
            ax4.bar(x_qp - width_qp/2, qp_activation_rates, width_qp, 
                   label='QP Activation Rate', color='orange', alpha=0.7)
            ax4.bar(x_qp + width_qp/2, qp_skip_rates, width_qp,
                   label='QP Skip Rate', color='green', alpha=0.7)
        ...
        if first_successful_scenario:
            ...
            for method, method_label, color in zip(methods, method_labels, colors):
                if method in results and results[method]['success'] and results[method]['M'] is not None:
                    ...
                    ax5.plot(x_grid, M[-1, :], label=f'{method_label} (t=T)', 
            ...
            for method, method_label, color in zip(methods, method_labels, colors):
                if method in results and results[method]['success'] and results[method]['U'] is not None:
                    ...
                    ax6.plot(x_grid, U[0, :], label=f'{method_label} (t=0)', 
                            color=color, linewidth=2)
        ...
        for method, method_label, color in zip(methods, method_labels, colors):
            ...
            if times and mass_errors:
                ax7.scatter(times, mass_errors, label=method_label, color=color, 
                           s=100, alpha=0.7)
        ...
        for method, label in zip(methods, method_labels):
            ...
            success_score = sum(1 for scenario_results in self.results.values() 
                               if method in scenario_results and scenario_results[method]['success'])
            
            # Average time (lower is better) 
            times = [r[method]['solve_time_minutes'] for r in self.results.values() 
                    if method in r and r[method]['success']]
            avg_time = np.mean(times) if times else float('inf')
            
            # Average mass error (lower is better)
            mass_errors = [r[method]['mass_error_percent'] for r in self.results.values()
        ...
        ax9.text(0.05, 0.95, ranking_text, transform=ax9.transAxes, fontsize=10,
                verticalalignment='top', fontfamily='monospace')
        
        plt.tight_layout()
        plt.savefig('/Users/zvezda/Library/CloudStorage/OneDrive-Personal/code/MFG_PDE/tests/method_comparisons/comprehensive_three_method_evaluation_v2.png', 
