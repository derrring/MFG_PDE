...
def run_fast_comparison():
    ...
    try:
        ...
        U_hybrid, M_hybrid, iters_hybrid, rel_distu_h, rel_distm_h = hybrid_iterator.solve(
            shared_settings["max_iterations"], 
            shared_settings["convergence_tolerance"]
        )
    ...
    try:
        ...
        U_qp, M_qp, solve_info = qp_solver.solve(
            Niter=shared_settings["max_iterations"],
            l2errBound=shared_settings["convergence_tolerance"],
            verbose=False
        )
...
def analyze_fast_results(results, problem):
    ...
    # Mass conservation
    both_reasonable_mass = (abs(hybrid['mass_change_percent']) < 10 and 
                           abs(qp['mass_change_percent']) < 10)
