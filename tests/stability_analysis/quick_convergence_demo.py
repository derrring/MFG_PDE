    ...
    for i, config in enumerate(test_configs):
        ...
        try:
            ...
            U_solution, M_solution, info = solver.solve(
                ...
                l2errBound=1e-3,  # Slightly relaxed for speed
                verbose=False
            )
...
def analyze_quick_convergence(results):
    ...
    for result in successful_results:
        print(f"{result['name']:<20} {result['final_mass']:<12.6f} {result['mass_change_percent']:<15.3f} "
...
def create_quick_convergence_plots(results):
    ...
    bars1 = ax3.bar(x_pos - width/2, norm_masses, width, 
                    label='Final Mass (normalized)', color='blue', alpha=0.7)
    bars2 = ax3.bar(x_pos + width/2, norm_centers, width, 
                    label='Center of Mass (normalized)', color='red', alpha=0.7)
    ...
    im = ax6.imshow(M_solution.T, aspect='auto', origin='lower', 
    ...
    plt.tight_layout()
    plt.savefig('/Users/zvezda/Library/CloudStorage/OneDrive-Personal/code/MFG_PDE/quick_convergence_demo.png', 
