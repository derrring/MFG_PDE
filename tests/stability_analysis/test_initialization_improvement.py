...
def test_initialization_improvement():
    ...
    problem = ExampleMFGProblem(
        xmin=0.0, xmax=1.0,
        Nx=50, T=1.0, Nt=25,  # Longer time with moderate temporal resolution
        sigma=0.25, coefCT=0.08  # More conservative for longer simulation
    )
    ...
    U, M, info = solver.solve(Niter=8, l2errBound=5e-3, verbose=True)  # More iterations and relaxed tolerance for longer sim
