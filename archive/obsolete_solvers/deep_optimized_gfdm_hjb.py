...
class DeepOptimizedGFDMHJBSolver(GFDMHJBSolver):
    ...
    def __init__(self, problem, collocation_points, delta=0.35, taylor_order=2,
        ...
        self.interior_points = [i for i in range(len(collocation_points)) 
                               if i not in self.boundary_point_set]
    ...
    def _enhanced_monotonicity_check(self, unconstrained_coeffs: np.ndarray, 
                                   point_idx: int, newton_iter: int, 
                                   time_ratio: float) -> bool:
        """
    ...
    def _solve_qp_with_warm_start(self, taylor_data: Dict, b: np.ndarray, 
                                point_idx: int, warm_start: Optional[np.ndarray] = None) -> np.ndarray:
    ...
    def approximate_derivatives(self, u_values: np.ndarray, point_idx: int) -> Dict[Tuple[int, ...], float]:
        ...
        if self.use_monotone_constraints and self.optimization_level > 0:
            ...
            needs_qp = self._enhanced_monotonicity_check(
                unconstrained_coeffs, point_idx, newton_iter, time_ratio
            )
            ...
            if not needs_qp:
                ...
                derivative_coeffs = self._solve_qp_with_warm_start(
    ...
    def _build_rhs_vector_for_point(self, point_idx: int, u_values: np.ndarray) -> np.ndarray:
        ...
        try:
            if hasattr(self, 'neighborhoods') and point_idx in self.neighborhoods:
                ...
                rhs = np.array([u_values[i] if i >= 0 and i < len(u_values) else 0.0 
    ...
    def print_performance_summary(self):
        ...
        print(f"  Unconstrained Solutions: {stats['unconstrained_solutions_used']} ({stats['unconstrained_usage_rate']:.1%})")
