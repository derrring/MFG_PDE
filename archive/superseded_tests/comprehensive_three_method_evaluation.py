...
def test_pure_fdm_method(problem_params, solver_params, test_label):
    ...
    try:
        ...
        hjb_solver = FdmHJBSolver(
            problem, NiterNewton=solver_params["newton_iterations"], l2errBoundNewton=solver_params["newton_tolerance"]
        )
...
def create_comprehensive_visualization(results, scenarios):
    ...
    for method in methods:
        ...
        if successful_indices:
            mass_errors = [
                method_data[method]['mass_error'][i]
                for i in successful_indices
                if method_data[method]['mass_error'][i] < 50
            ]
            times = [
                method_data[method]['time'][i] for i in successful_indices if method_data[method]['mass_error'][i] < 50
            ]

            if mass_errors and times:
                ax4.scatter(times, mass_errors, label=method, color=method_colors[method], alpha=0.7, s=60)
    ...
    for i, method in enumerate(methods):
        ...
        ax5.bar(x_pos + i * width, rates, width, label=method, color=method_colors[method], alpha=0.7)
    ...
    plt.tight_layout()
    plt.savefig(
        '/Users/zvezda/Library/CloudStorage/OneDrive-Personal/code/MFG_PDE/tests/method_comparisons/comprehensive_three_method_evaluation.png',
        dpi=150,
        bbox_inches='tight',
    )
    plt.show()
