...
def compare_qp_method_analysis():
    ...
    problem_params = {"xmin": 0.0, "xmax": 1.0, "Nx": 25, "T": 0.2, "Nt": 10, "sigma": 0.2, "coefCT": 0.01}
    ...
    for config in configs:
        ...
        try:
            ...
            # Collocation setup
            collocation_points = np.linspace(problem.xmin, problem.xmax, config['num_collocation']).reshape(-1, 1)
    ...
    if len(successful_configs) > 0:
        ...
        for config_name in successful_configs:
            ...
            print(
                f"{config_name:<20} {result['mass_loss_percent']:<12.3f} {result['mass_quality']:<12} {result['time']:<12.2f} {result['violations']:<12}"
            )
        ...
        print(f"\n--- Performance Winners ---")
        print(
            f"🏆 Best mass conservation: {best_mass_conservation} ({results[best_mass_conservation]['mass_loss_percent']:.3f}% loss)"
        )
        ...
        mass_increasing_configs = [
            name for name in successful_configs if results[name]['final_mass'] > results[name]['initial_mass']
        ]
...
def create_qp_analysis_plots(results, problem, successful_configs):
    ...
    for i, config_name in enumerate(successful_configs):
        ...
        ax1.plot(problem.tSpace, mass_evolution, label=config_name, linewidth=2, color=colors[i % len(colors)])
    ...
    for i, config_name in enumerate(successful_configs):
        ...
        ax2.plot(problem.xSpace, final_density, label=config_name, linewidth=2, color=colors[i % len(colors)])
    ...
    for bar, value in zip(bars, runtimes):
        ax4.text(bar.get_x() + bar.get_width() / 2.0, bar.get_height(), f'{value:.2f}s', ha='center', va='bottom')
    plt.setp(ax4.get_xticklabels(), rotation=45)

    plt.tight_layout()
    plt.savefig(
        '/Users/zvezda/Library/CloudStorage/OneDrive-Personal/code/MFG_PDE/qp_analysis.png',
        dpi=150,
        bbox_inches='tight',
    )
    plt.show()
