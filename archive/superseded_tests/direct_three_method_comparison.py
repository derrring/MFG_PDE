...
def compare_three_methods_direct():
    ...
    problem_params = {"xmin": 0.0, "xmax": 1.0, "Nx": 20, "T": 0.15, "Nt": 8, "sigma": 0.15, "coefCT": 0.005}
    ...
    try:
        start_time = time.time()
        hybrid_solver = HybridSolver(problem, num_particles=500, thetaUM=0.7, NiterNewton=5, l2errBoundNewton=1e-3)
    ...
    try:
        ...
        U_colloc, M_colloc, info_colloc = collocation_solver.solve(Niter=6, l2errBound=1e-3, verbose=False)
...
def create_direct_comparison_plots(results, problem, successful_methods):
    ...
    for bar, value in zip(bars, mass_changes):
        ax3.text(bar.get_x() + bar.get_width() / 2.0, bar.get_height(), f'{value:.1e}', ha='center', va='bottom')
    ...
    plt.tight_layout()
    plt.savefig(
        '/Users/zvezda/Library/CloudStorage/OneDrive-Personal/code/MFG_PDE/direct_comparison.png',
        dpi=150,
        bbox_inches='tight',
    )
    plt.show()
