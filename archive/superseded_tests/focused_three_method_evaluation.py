    ...
    try:
        ...
        if method_name == "Pure FDM":
            ...
            iterator = FixedPointIterator(
                problem, hjb_solver=hjb_solver, fp_solver=fp_solver, 
            ...
            fp_solver = ParticleFPSolver(
                problem, num_particles=solver_params["num_particles"],
                kde_bandwidth="scott", normalize_kde_output=False,
            ...
            qp_solver = ParticleCollocationSolver(
                problem=problem, collocation_points=collocation_points,
                num_particles=solver_params["num_particles"], delta=0.35,
                taylor_order=2, weight_function="wendland",
                NiterNewton=solver_params["newton_iterations"],
                l2errBoundNewton=solver_params["newton_tolerance"],
                kde_bandwidth="scott", normalize_kde_output=False,
                boundary_indices=boundary_indices, boundary_conditions=no_flux_bc,
        ...
        if total_time > timeout_seconds:
            return {
                'success': False, 'method': method_name, 'error': 'Timeout',
...
def run_focused_evaluation():
    ...
    test_scenarios = [
        ...
        # Standard case
        {
            ...
            'solver': {'max_iterations': 12, 'convergence_tolerance': 1e-3, 'newton_iterations': 8, 'newton_tolerance': 1e-4, 'num_particles': 400, 'thetaUM': 0.4}
        },
        # Moderate challenge
        {
            'name': 'Challenge',
        ...
        # Demanding case
        {
            ...
            'solver': {'max_iterations': 15, 'convergence_tolerance': 1e-3, 'newton_iterations': 10, 'newton_tolerance': 1e-4, 'num_particles': 600, 'thetaUM': 0.3}
        }
    ]
...
def analyze_focused_results(results):
    ...
    for method in methods:
        ...
        if method_results:
            ...
            print(f"{method:<15}: Avg {avg_time:.2f}s, Median {median_time:.2f}s, Range [{min_time:.2f}s, {max_time:.2f}s]")
...
def create_focused_visualization(results, scenarios):
    ...
    for i, method in enumerate(methods):
        ...
        for scenario_name in scenario_names:
            scenario_result = next((r for r in results if r.get('method') == method and r.get('scenario') == scenario_name), None)
        ...
        ax4.bar(x_pos + i*width, scenario_times, width, label=method, 
    ...
    for method in methods:
        ...
        if method_results:
            ...
            ax5.scatter(times, mass_errors, label=method, color=method_colors[method], 
    ...
    ax5.text(0.02, 0.95, 'Excellent\nMass Conservation\n(<1%)', transform=ax5.transAxes, 
             bbox=dict(boxstyle="round,pad=0.3", facecolor="lightgreen", alpha=0.7), fontsize=8)
    ax5.text(0.02, 0.02, 'Fast\nComputation\n(<10s)', transform=ax5.transAxes, 
    ...
    for method in methods:
        ...
        if method_results:
            ...
            summary_data.append([method, f"{success_rate:.0f}%", f"{avg_mass_error:.3f}%", f"{avg_time:.1f}s", f"{convergence_rate:.0f}%"])
        else:
    ...
    plt.tight_layout()
    plt.savefig('/Users/zvezda/Library/CloudStorage/OneDrive-Personal/code/MFG_PDE/tests/method_comparisons/focused_three_method_evaluation.png', 
