        ...
        for t_idx in range(Nt):
            ...
            if len(np.unique(particles_t)) > 1:
                try:
                    ...
                    hist, edges = np.histogram(particles_t, bins=Nx, range=(self.problem.xmin, self.problem.xmax))
...
def compare_three_methods_fixed():
    ...
    problem_params = {"xmin": 0.0, "xmax": 1.0, "Nx": 30, "T": 0.3, "Nt": 15, "sigma": 0.3, "coefCT": 0.02}
    ...
    try:
        ...
        U_colloc, M_colloc, info_colloc = collocation_solver.solve(Niter=15, l2errBound=5e-4, verbose=False)
...
def create_fixed_comparison_plots(results, problem, successful_methods):
    ...
    plt.tight_layout()
    plt.savefig(
        '/Users/zvezda/Library/CloudStorage/OneDrive-Personal/code/MFG_PDE/fixed_comparison.png',
        dpi=150,
        bbox_inches='tight',
    )
    plt.show()
