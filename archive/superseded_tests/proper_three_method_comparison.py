...
def compare_three_methods_proper():
    ...
    try:
        ...
        U_fdm, M_fdm, iterations_fdm, l2dist_u, l2dist_m = fdm_solver.solve(
            Niter=15, l2errBoundPicard=1e-4  # Conservative Picard iterations
        )
        ...
        if M_fdm is not None and U_fdm is not None:
            ...
            results['fdm'] = {
                ...
                'arrays': {'U_solution': U_fdm, 'M_solution': M_fdm, 'mass_evolution': mass_evolution_fdm},
            }

            print(f"  ✓ FDM completed successfully in {fdm_time:.2f}s")
            print(f"    Iterations: {iterations_fdm}")
            print(
                f"    Mass: {initial_mass_fdm:.6f} → {final_mass_fdm:.6f} ({(mass_change_fdm/initial_mass_fdm)*100:+.3f}%)"
            )
    ...
    try:
        ...
        U_hybrid, M_hybrid, iterations_hybrid, l2dist_u_h, l2dist_m_h = hybrid_solver.solve(
            Niter=15, l2errBoundPicard=1e-4  # Match FDM iterations
        )
    ...
    try:
        ...
        U_qp, M_qp, info_qp = qp_solver.solve(Niter=15, l2errBound=1e-4, verbose=False)  # Match other methods
...
def analyze_method_convergence(results, problem):
    ...
    successful_methods = [method for method in ['fdm', 'hybrid', 'qp'] if results.get(method, {}).get('success', False)]
    ...
    for method in successful_methods:
        ...
        print(
            f"{result['method_name']:<25} {mass_cons['final_mass']:<12.6f} "
    ...
    all_conservative = all(
        abs(results[m]['mass_conservation']['mass_change_percent']) < 5.0 for m in successful_methods
    )
    ...
    all_clean = all(
        results[m]['solution_quality']['violations'] == 0 and results[m]['solution_quality']['negative_densities'] == 0
...
def create_convergence_comparison_plots(results, problem):
    ...
    for method in successful_methods:
        ...
        ax2.plot(problem.tSpace, mass_evolution, 'o-', label=method_names[method], color=colors[method], linewidth=2)
    ...
    bars1 = ax3.bar(x_pos - width / 2, centers_of_mass, width, label='Center of Mass', alpha=0.8, color='purple')
    bars2 = ax3.bar(x_pos + width / 2, max_density_locs, width, label='Max Density Location', alpha=0.8, color='orange')
    ...
    for bar, value in zip(bars1, centers_of_mass):
        ax3.text(
            bar.get_x() + bar.get_width() / 2.0, bar.get_height(), f'{value:.3f}', ha='center', va='bottom', fontsize=9
        )
    ...
    if len(successful_methods) >= 2:
        ...
        scatter = ax5.scatter(
            times, accuracy_scores, c=[colors[m] for m in successful_methods], s=150, alpha=0.7, edgecolors='black'
        )

        for i, method in enumerate(successful_methods):
            ax5.annotate(
                method_names[method],
                (times[i], accuracy_scores[i]),
                xytext=(5, 5),
    ...
    plt.tight_layout()
    plt.savefig(
        '/Users/zvezda/Library/CloudStorage/OneDrive-Personal/code/MFG_PDE/proper_three_method_comparison.png',
        dpi=150,
        bbox_inches='tight',
    )
    plt.show()
