...
class SimpleHybridSolver(MFGSolver):
    ...
    def __init__(self, problem, num_particles, kde_bandwidth="scott", NiterNewton=10, l2errBoundNewton=1e-4):
    ...
    def _estimate_density_from_particles(self, particles):
        ...
        try:
            ...
            hist, _ = np.histogram(particles, bins=grid_size, range=(self.problem.xmin, self.problem.xmax))
    ...
    def _solve_hjb_step(self, t_idx, M_current):
        ...
        for newton_iter in range(self.NiterNewton):
            ...
            # Solve linear system
            A = sparse.diags([diag_off, diag_main, diag_off], [-1, 0, 1], shape=(grid_size, grid_size), format='csr')
    ...
    def solve(self, Niter=10, l2errBoundPicard=1e-4):
        ...
        for picard_iter in range(Niter):
            ...
            for t_idx in range(1, Nt + 1):
                ...
                self.M_density[t_idx, :] = self._estimate_density_from_particles(self.M_particles[t_idx, :])
...
def compare_hybrid_vs_qp():
    ...
    # Problem parameters
    problem_params = {"xmin": 0.0, "xmax": 1.0, "Nx": 30, "T": 0.2, "Nt": 10, "sigma": 0.15, "coefCT": 0.01}

    # Shared settings
    shared_settings = {"num_particles": 600, "newton_iterations": 6, "picard_iterations": 10, "tolerance": 1e-4}
    ...
    try:
        ...
        U_hybrid, M_hybrid, iterations_hybrid, l2dist_u, l2dist_m = hybrid_solver.solve(
            Niter=shared_settings["picard_iterations"], l2errBoundPicard=shared_settings["tolerance"]
        )
        ...
        violations_hybrid = np.sum((final_particles < problem.xmin - 1e-10) | (final_particles > problem.xmax + 1e-10))
    ...
    try:
        ...
        U_qp, M_qp, info_qp = qp_solver.solve(
            Niter=shared_settings["picard_iterations"], l2errBound=shared_settings["tolerance"], verbose=False
        )
        ...
        if particles_traj is not None:
            ...
            violations_qp = np.sum((final_particles < problem.xmin - 1e-10) | (final_particles > problem.xmax + 1e-10))
...
def analyze_comparison(results):
    ...
    mass_diff = abs(qp['mass_conservation']['mass_change_percent'] - hybrid['mass_conservation']['mass_change_percent'])
    com_diff = abs(qp['physical_observables']['center_of_mass'] - hybrid['physical_observables']['center_of_mass'])

    print(f"Mass change difference: {mass_diff:.2f} percentage points")
    print(f"Center of mass difference: {com_diff:.4f}")

    # Check consistency
    both_increase = hybrid['mass_conservation']['mass_change'] > 0 and qp['mass_conservation']['mass_change'] > 0
    both_clean = (
        hybrid['solution_quality']['violations'] == 0
        and qp['solution_quality']['violations'] == 0
        and hybrid['solution_quality']['negative_densities'] == 0
        and qp['solution_quality']['negative_densities'] == 0
    )
...
def create_comparison_plots(results, problem):
    ...
    ax1 = axes[0, 0]
    ax1.plot(problem.xSpace, hybrid['arrays']['M_solution'][-1, :], 'g-', linewidth=2, label='Hybrid')
    ax1.plot(problem.xSpace, qp['arrays']['M_solution'][-1, :], 'r-', linewidth=2, label='QP-Collocation')
    ...
    # 2. Mass evolution
    ax2 = axes[0, 1]
    ax2.plot(problem.tSpace, hybrid['arrays']['mass_evolution'], 'g-o', linewidth=2, label='Hybrid')
    ax2.plot(problem.tSpace, qp['arrays']['mass_evolution'], 'r-s', linewidth=2, label='QP-Collocation')
    ...
    qp_vals = [qp['physical_observables']['center_of_mass'], qp['physical_observables']['max_density_location']]
    ...
    plt.tight_layout()
    plt.savefig(
        '/Users/zvezda/Library/CloudStorage/OneDrive-Personal/code/MFG_PDE/simple_hybrid_vs_qp.png',
        dpi=150,
        bbox_inches='tight',
    )
    plt.show()
