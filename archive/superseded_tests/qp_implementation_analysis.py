...
class QP_ImplementationProfiler:
    ...
    def setup_problem(self):
        ...
        collocation_points = np.linspace(
            self.problem.xmin, self.problem.xmax, num_collocation_points
        ).reshape(-1, 1)
        ...
        for i, point in enumerate(collocation_points):
            x = point[0]
            if (abs(x - self.problem.xmin) < boundary_tolerance or 
                abs(x - self.problem.xmax) < boundary_tolerance):
    ...
    def test_qp_overhead(self):
        ...
        start_time = time.time()
        try:
            U_qp, M_qp, info_qp = self.solver.solve(
                Niter=3, l2errBound=1e-2, verbose=False
            )
        ...
        start_time = time.time()
        try:
            U_no_qp, M_no_qp, info_no_qp = self.solver.solve(
                Niter=3, l2errBound=1e-2, verbose=False
            )
...
def run_qp_implementation_analysis():
    ...
    problem_params = {
        'xmin': 0.0, 'xmax': 1.0, 'Nx': 25, 'T': 1.0, 'Nt': 50,
