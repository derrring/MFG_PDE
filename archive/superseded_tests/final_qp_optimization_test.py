...
def print_comprehensive_summary(results):
    ...
    for key, result in results.items():
        ...
        print(
            f"{result['method']:<35} {success_str:<8} {time_str:<10} {mass_str:<12} {qp_usage_str:<12} {speedup_str:<10} {status:<15}"
        )
...
def create_comprehensive_plots(results, problem_params):
    ...
    for bar, time_val in zip(bars2, times):
        height = bar.get_height()
        ax2.text(bar.get_x() + bar.get_width() / 2.0, height + 5, f'{time_val:.1f}s', ha='center', va='bottom')
    ...
    if baseline_time:
        ...
        for bar, speedup in zip(bars3, speedups):
            height = bar.get_height()
            ax3.text(bar.get_x() + bar.get_width() / 2.0, height + 0.05, f'{speedup:.2f}x', ha='center', va='bottom')
    ...
    if len(successful_results) > 1:
        ...
        for i, (method, qp_rate, solve_time) in enumerate(zip(methods, qp_usage_rates, solve_times)):
            ...
            ax5.annotate(method, (qp_rate, solve_time), xytext=(5, 5), textcoords='offset points', fontsize=8)
    ...
    for i, (key, result) in enumerate(successful_results.items()):
        M = result['M']
        ax6.plot(x_grid, M[-1, :], label=f"{result['method']} (final)", color=colors[i], linewidth=2)
    ...
    plt.tight_layout()
    plt.savefig(
        '/Users/zvezda/Library/CloudStorage/OneDrive-Personal/code/MFG_PDE/tests/method_comparisons/comprehensive_qp_optimization_results.png',
        dpi=300,
        bbox_inches='tight',
    )
