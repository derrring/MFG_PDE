...
from mfg_pde.utils.convergence import (
    adaptive_convergence, wrap_solver_with_adaptive_convergence, 
...
def demonstrate_adaptive_convergence():
    ...
    adaptive_fdm = wrap_solver_with_adaptive_convergence(
        pure_fdm_solver,
        classical_tol=1e-3,
        verbose=True
    )
    ...
    create_adaptive_convergence_comparison(
        [U_fdm, U_particle], [M_fdm, M_particle], 
        ['Pure FDM (Classical)', 'Particle Collocation (Advanced)'],
        problem
    )
...
def demonstrate_detection_capabilities():
    ...
    particle_collocation = ParticleCollocationSolver(
        problem=problem, collocation_points=points, num_particles=1000
    )
