...
def compare_three_core_methods():
    ...
    try:
        ...
        fdm_solver = FDMSolver(problem=problem, thetaUM=0.5, NiterNewton=30, l2errBoundNewton=1e-6)  # Damping parameter

        U_fdm, M_fdm = fdm_solver.solve(Niter=12, l2errBoundPicard=2e-3)  # Conservative iteration count
        ...
        if M_fdm is not None and U_fdm is not None:
            ...
            # Check convergence
            converged_fdm = (
                fdm_solver.l2distu is not None and len(fdm_solver.l2distu) > 0 and fdm_solver.l2distu[-1] < 2e-3
            )
    ...
    try:
        ...
        U_hybrid, M_hybrid = hybrid_solver.solve(Niter=12, l2errBoundPicard=2e-3)
        ...
        if M_hybrid is not None and U_hybrid is not None:
            ...
            # Check convergence
            converged_hybrid = (
                hybrid_solver.l2distu is not None
                and len(hybrid_solver.l2distu) > 0
                and hybrid_solver.l2distu[-1] < 2e-3
            )
    ...
    try:
        ...
        U_colloc, M_colloc, info_colloc = collocation_solver.solve(Niter=12, l2errBound=2e-3, verbose=True)
...
def create_comparison_plots(results, problem, successful_methods):
    ...
    for method in successful_methods:
        ...
        ax2.plot(problem.xSpace, final_value, label=method_names[method], linewidth=2, color=colors[method])
    ...
    for method in successful_methods:
        ...
        ax3.plot(problem.tSpace, mass_evolution, label=method_names[method], linewidth=2, color=colors[method])
    ...
    for bar, value in zip(bars, mass_changes):
        ...
        ax4.text(bar.get_x() + bar.get_width() / 2.0, height, f'{value:.1e}', ha='center', va='bottom')
    ...
    for bar, value in zip(bars, runtimes):
        height = bar.get_height()
        ax5.text(bar.get_x() + bar.get_width() / 2.0, height, f'{value:.1f}s', ha='center', va='bottom')
    ...
    if particle_methods:
        ...
        for bar, value in zip(bars, violations):
            height = bar.get_height()
            ax6.text(bar.get_x() + bar.get_width() / 2.0, height, f'{value}', ha='center', va='bottom')
    else:
