    ...
    try:
        ...
        if U_hybrid is not None and M_hybrid is not None:
            ...
            result = {
                'success': True,
                'label': label,
                'params': {'T': T, 'thetaUM': thetaUM, 'sigma': sigma, 'coefCT': coefCT, 
                          'particles': num_particles, 'newton_iter': newton_iterations},
                'time': solve_time,
...
def run_mass_loss_analysis():
    ...
    print(f"  Aggressive damping (>0.5): {len(aggressive_damping_loss)}/{len(aggressive_damping)} show initial mass loss")
...
def create_mass_loss_plots(results):
    ...
    for r in smooth_cases[:5]:  # Limit to first 5
        ax1.plot(r['time_grid'], r['mass_evolution'], 'g-', alpha=0.7, 
    ...
    for r in mass_loss_cases[:5]:  # Limit to first 5
        ax2.plot(r['time_grid'], r['mass_evolution'], 'r-', alpha=0.7,
                label=f"{r['label'][:15]}... ({r['early_mass_loss_percent']:.2f}%)")
    ...
    plt.tight_layout()
    plt.savefig('/Users/zvezda/Library/CloudStorage/OneDrive-Personal/code/MFG_PDE/mass_loss_analysis.png', 
