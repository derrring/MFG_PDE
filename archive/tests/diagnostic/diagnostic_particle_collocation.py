...
def test_mapping_accuracy():
    ...
    solver = ParticleCollocationSolver(
        problem=problem, collocation_points=collocation_points, num_particles=500, delta=0.2, taylor_order=2
    )
...
def compare_with_fdm():
    ...
    U_fdm = fdm_solver.solve_hjb_system(
        M_density_evolution=M_initial, U_final_condition=U_initial[Nt - 1, :], U_from_prev_picard=U_initial
    )
...
def create_diagnostic_plots(U_fdm, U_gfdm):
    ...
    axes[1, 1].bar(
        ['FDM min', 'FDM max', 'GFDM min', 'GFDM max'], [fdm_range[0], fdm_range[1], gfdm_range[0], gfdm_range[1]]
    )
