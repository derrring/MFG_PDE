...
class BaseMFGSolver(metaclass = ABCMeta):
    ...
    def _discretization(self):
        ...
        self.u_store[-2] = [0.21486721, 0.2075614,   0.1245926,   0.00281925, -0.1184688, -0.20901957, -0.25166382, -0.24016895, -0.17612104, -0.06958008,  0.0581872,   0.17576343, 0.24951195,  0.25642903,  0.1949944,   0.08583865, -0.03821726, -0.1448157, -0.21039671, -0.22266705, -0.17972463, -0.08999091,  0.02580829,  0.13392367, 0.19237932]
        self.m_store[1] = np.array([0.        , 0.        , 0.        , 0.        , 0.        ,
        0.        , 0.        , 0.17474761, 0.66402542, 1.46055943,
        2.49867314, 3.52565227, 4.17634213, 4.17634213, 3.52565227,
        2.49867314, 1.46055943, 0.66402542, 0.17474761, 0.        ,
        0.        , 0.        , 0.        , 0.        , 0.        ])
    ...
    def solve_forward_m(self, u_current):
        ...
        for k in range(1, self.N + 1):
            ...
            for i in range(1, M-1):
                ...
                md[i] += 0.5 * (np.maximum(u_km1[i+1] - u_km1[i], 0) + 
                            np.minimum(u_km1[i] - u_km1[i-1], 0)) / (dx**2)
            ...
            matrix = sp.sparse.spdiags(
                [ml_corner, ml, md, mu, mu_corner],
                [M-1, -1, 0, 1, -(M-1)],
                M, M
            ).tocsr()
...
class ConservativeMFGSolver(BaseMFGSolver):
    ...
    def _setup_basic_matrices(self):
        ...
        self.A_u = -1/(4*dt)*(diag_ones+shift_right) - sigma/(2*dx**2) * (-2*diag_ones + shift_right + shift_left)

        self.B_u = -1/(4*dt)*(-diag_ones+shift_right)

        self.C_u = -1/(4*dt)*shift_right-  sigma/(2*dx**2)*(-2*diag_ones+shift_right+shift_left)

        self.A_m = 1/(4*dt)*(diag_ones+shift_right) - sigma/(2*dx**2)*(-2*diag_ones+shift_right+shift_left)
    ...
    def _setup_boundary_matrices(self):
        ...
        if self.boundary.type == 'periodic':
            ...
            if not hasattr(self.boundary, 'left_alpha') or \
                any(v is None for v in [self.boundary.left_alpha, self.boundary.left_beta,
                                        self.boundary.right_alpha, self.boundary.right_beta]):
                raise ValueError("Robin boundary conditions require alpha and beta coefficients")
