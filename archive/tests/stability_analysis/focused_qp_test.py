...
def test_qp_mass_conservation():
    ...
    for i, test_case in enumerate(test_cases):
        ...
        try:
            ...
            U_solution, M_solution, info = solver.solve(
                Niter=test_case['qp_settings']['picard'], l2errBound=1e-2, verbose=False
            )
    ...
    if len(successful_results) > 0:
        ...
        for result in successful_results:
            print(
                f"{result['name']:<20} {result['mass_change']:+.2e} {result['mass_behavior']:<12} {result['mass_status']:<15} {result['violations']:<12}"
            )
        ...
        if len(successful_results) >= 2:
            ...
            normalized_masses = [
                fm / (r['problem'].xmax - r['problem'].xmin) for fm, r in zip(final_masses, successful_results)
            ]
...
def create_mass_conservation_plots(results):
    ...
    for i, result in enumerate(results):
        if 'M_solution' in result:
            ...
            ax3.plot(problem.xSpace, final_density, label=result['name'], color=colors[i % len(colors)], linewidth=2)
    ...
    ax4.grid(True)
    for i, result in enumerate(results):
        ax4.annotate(result['name'], (runtimes[i], mass_changes_abs[i]), xytext=(5, 5), textcoords='offset points')

    plt.tight_layout()
    plt.savefig(
        '/Users/zvezda/Library/CloudStorage/OneDrive-Personal/code/MFG_PDE/mass_conservation_analysis.png',
        dpi=150,
        bbox_inches='tight',
    )
    plt.show()
