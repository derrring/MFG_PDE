...
def analyze_final_density_convergence():
    ...
    base_params = {"xmin": 0.0, "xmax": 1.0, "T": 0.3, "sigma": 0.2, "coefCT": 0.02}
    ...
    for i, scenario in enumerate(test_scenarios):
        ...
        print(
            f"  Particles: {scenario['qp_settings']['particles']}, Collocation: {scenario['qp_settings']['collocation']}"
        )
        ...
        try:
            ...
            U_solution, M_solution, info = solver.solve(
                Niter=scenario['qp_settings']['picard'], l2errBound=1e-4, verbose=False
            )
            ...
            if M_solution is not None and U_solution is not None:
                ...
                result = {
                    ...
                    'success': True,
                    'scenario': scenario,
                    'timing': {'solve_time': solve_time, 'total_time': total_scenario_time},
                    'convergence': {
                        'converged': converged,
                ...
                print(
                    f"  Density profile: [0.25: {density_at_quarter:.3f}, 0.5: {density_at_half:.3f}, 0.75: {density_at_three_quarter:.3f}]"
                )
                ...
                results.append(
                    {'name': scenario['name'], 'success': False, 'total_time': time.time() - scenario_start_time}
                )

        except Exception as e:
            error_time = time.time() - scenario_start_time
            print(f"❌ Crashed after {error_time:.2f}s: {e}")
            results.append({'name': scenario['name'], 'success': False, 'error': str(e), 'total_time': error_time})
...
def analyze_density_variations(results):
    ...
    for result in successful_results:
        ...
        print(
            f"{result['name']:<20} {phys['center_of_mass']:<15.6f} "
...
def create_density_variation_plots(results):
    ...
    for i, result in enumerate(successful_results):
        ...
        if i == 0:
            ...
            ax1.plot(
                problem.xSpace,
                final_density,
                label=f"{result['name']} (Reference)",
                color=colors[i],
                linewidth=3,
                alpha=0.9,
            )
        else:
            ax1.plot(problem.xSpace, final_density, label=result['name'], color=colors[i], linewidth=2, alpha=0.7)
    ...
    if reference_density is not None:
        for i, result in enumerate(successful_results[1:], 1):  # Skip reference
            ...
            ax2.plot(ref_problem.xSpace, density_diff, label=result['name'], color=colors[i], linewidth=2)
    ...
    bars1 = ax3.bar(x_pos - width / 2, centers_of_mass, width, label='Center of Mass', alpha=0.8, color='blue')
    bars2 = ax3.bar(x_pos + width / 2, max_density_locs, width, label='Max Density Location', alpha=0.8, color='red')
    ...
    for i, result in enumerate(successful_results):
        ...
        ax5.plot(problem.tSpace, mass_evolution, 'o-', label=result['name'], color=colors[i], linewidth=2, alpha=0.8)
    ...
    # Scatter plot
    scatter1 = ax6.scatter(execution_times, com_rel_diffs, c='blue', alpha=0.7, s=100, label='Center of Mass')
    scatter2 = ax6.scatter(execution_times, peak_rel_diffs, c='red', alpha=0.7, s=100, label='Peak Density')
    ...
    for i, result in enumerate(successful_results):
        if i > 0:  # Skip reference point
            ax6.annotate(
                result['name'],
                (execution_times[i], max(com_rel_diffs[i], peak_rel_diffs[i])),
                xytext=(5, 5),
    ...
    plt.tight_layout()
    plt.savefig(
        '/Users/zvezda/Library/CloudStorage/OneDrive-Personal/code/MFG_PDE/final_density_convergence_analysis.png',
        dpi=150,
        bbox_inches='tight',
    )
    plt.show()
