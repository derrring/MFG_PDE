    ...
    test_configs = [
        {
            ...
            'params': {'xmin': 0.0, 'xmax': 1.0, 'Nx': 40, 'T': 1.0, 'Nt': 25, 'sigma': 0.2, 'coefCT': 0.02},
            'qp_settings': {'particles': 800, 'collocation': 15, 'delta': 0.3, 'order': 2, 'newton': 8, 'picard': 12},
        },
        {
            'name': 'Ultra High-Res',
            'params': {'xmin': 0.0, 'xmax': 1.0, 'Nx': 60, 'T': 1.0, 'Nt': 40, 'sigma': 0.25, 'coefCT': 0.025},
            'qp_settings': {
                'particles': 1200,
                'collocation': 20,
                'delta': 0.35,
        ...
        },
        {
            ...
            'params': {'xmin': 0.0, 'xmax': 1.0, 'Nx': 80, 'T': 1.0, 'Nt': 50, 'sigma': 0.3, 'coefCT': 0.03},
            'qp_settings': {'particles': 1500, 'collocation': 25, 'delta': 0.4, 'order': 2, 'newton': 12, 'picard': 20},
        },
    ]
        ...
        try:
            ...
            if M_solution is not None and U_solution is not None:
                ...
                if particles_traj is not None:
                    ...
                    particle_in_bounds = np.sum((final_particles >= problem.xmin) & (final_particles <= problem.xmax))
...
def analyze_all_results(results):
    ...
    for result in successful_results:
        ...
        print(
            f"{result['name']:<20} {mass['mass_change_percent']:<15.3f} {mass['mass_variation_percent']:<12.3f} {assessment:<15}"
        )
    ...
    for result in successful_results:
        conv = result['convergence']
        print(
            f"{result['name']:<20} {str(conv['converged']):<12} {conv['iterations_used']:<12} {conv['convergence_rate']:<12.1f}"
        )
...
def create_high_resolution_plots(results):
    ...
    for i, result in enumerate(successful_results):
        ...
        ax1.plot(problem.tSpace, mass_evolution, 'o-', label=result['name'], color=colors[i % len(colors)], linewidth=2)
    ...
    for i, result in enumerate(successful_results):
        ...
        ax2.plot(problem.xSpace, final_density, label=result['name'], color=colors[i % len(colors)], linewidth=2)
    ...
    for bar, value in zip(bars, mass_changes):
        ax3.text(bar.get_x() + bar.get_width() / 2.0, bar.get_height(), f'{value:.2f}%', ha='center', va='bottom')
    ...
    for bar, value in zip(bars, solve_times):
        ax4.text(bar.get_x() + bar.get_width() / 2.0, bar.get_height(), f'{value:.1f}s', ha='center', va='bottom')
    ...
    for bar, value in zip(bars, convergence_rates):
        ax5.text(bar.get_x() + bar.get_width() / 2.0, bar.get_height(), f'{value:.1f}%', ha='center', va='bottom')
    ...
    scatter = ax7.scatter(solve_times, mass_accuracy, c=colors[: len(successful_results)], s=100, alpha=0.7)
    ax7.set_xlabel('Solve Time (seconds)')
    ax7.set_ylabel('|Mass Change %|')
    ax7.set_title('Performance vs Accuracy Trade-off')
    ax7.grid(True)
    for i, result in enumerate(successful_results):
        ax7.annotate(result['name'], (solve_times[i], mass_accuracy[i]), xytext=(5, 5), textcoords='offset points')

    # 8. Density evolution heatmap for best result (position 8)
    ax8 = plt.subplot(3, 3, 8)
    if successful_results:
        # Choose the result with best mass conservation
        best_result = min(successful_results, key=lambda x: abs(x['mass_conservation']['mass_change_percent']))
    ...
    if successful_results:
        ...
        if particles_traj is not None:
            final_particles = particles_traj[-1, :]
            ax9.hist(final_particles, bins=30, alpha=0.7, density=True, color=colors[0], edgecolor='black')
    ...
    plt.tight_layout()
    plt.savefig(
        '/Users/zvezda/Library/CloudStorage/OneDrive-Personal/code/MFG_PDE/high_resolution_t1_analysis.png',
        dpi=150,
        bbox_inches='tight',
    )
    plt.show()
...
def save_results_summary(results):
    ...
    # Save to JSON file
    with open(
        '/Users/zvezda/Library/CloudStorage/OneDrive-Personal/code/MFG_PDE/high_resolution_t1_results.json', 'w'
    ) as f:
