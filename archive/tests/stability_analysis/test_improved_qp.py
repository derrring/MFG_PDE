...
def test_improved_qp():
    ...
    problem = ExampleMFGProblem(
        xmin=0.0, xmax=1.0, Nx=40, T=0.5, Nt=20, sigma=0.3, coefCT=0.1  # More challenging problem
    )
    ...
    if M_std is not None and M_qp is not None:
        ...
        mass_var_imp = (
            (mass_variation_std - mass_variation_qp) / mass_variation_std * 100 if mass_variation_std > 0 else 0
        )
