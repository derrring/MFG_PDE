...
def test_working_noflux():
    ...
    problem = ExampleMFGProblem(xmin=0.0, xmax=1.0, Nx=8, T=0.05, Nt=3, sigma=0.3, coefCT=0.1)
    ...
    try:
        ...
        U, M, info = solver.solve(
            Niter=3, l2errBound=1e-2, verbose=True  # Only 3 Picard iterations  # Very relaxed convergence
        )
