...
def analyze_initialization():
    ...
    print(
        f"  U_current[0] (initial time): range [{U_current[0].min():.4f}, {U_current[0].max():.4f}], norm={np.linalg.norm(U_current[0]):.4f}"
    )
    print(
        f"  U_current[-1] (final time): range [{U_current[-1].min():.4f}, {U_current[-1].max():.4f}], norm={np.linalg.norm(U_current[-1]):.4f}"
    )

    print(f"  M_current shape: {M_current.shape}")
    print(
        f"  M_current[0] (initial time): range [{M_current[0].min():.4f}, {M_current[0].max():.4f}], norm={np.linalg.norm(M_current[0]):.4f}"
    )
    print(
        f"  M_current[-1] (final time): range [{M_current[-1].min():.4f}, {M_current[-1].max():.4f}], norm={np.linalg.norm(M_current[-1]):.4f}"
    )
