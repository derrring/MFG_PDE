...
class SimpleHybridSolver:
    ...
    def _estimate_density_from_particles(self, particles):
        ...
        for t_idx in range(Nt):
            ...
            valid_particles = particles_t[(particles_t >= self.problem.xmin) & (particles_t <= self.problem.xmax)]
            ...
            if len(valid_particles) > 5 and len(np.unique(valid_particles)) > 1:
                try:
                    ...
                    hist, _ = np.histogram(valid_particles, bins=Nx, range=(self.problem.xmin, self.problem.xmax))
...
def compare_three_methods_convergence():
    ...
    problem_params = {"xmin": 0.0, "xmax": 1.0, "Nx": 25, "T": 0.3, "Nt": 15, "sigma": 0.2, "coefCT": 0.015}
    ...
    try:
        ...
        if M_fdm is not None and U_fdm is not None:
            ...
            print(
                f"  Time: {time_fdm:.2f}s, Converged: {fdm_solver.converged}, Iterations: {fdm_solver.iterations_run}"
            )
        else:
    ...
    try:
        ...
        U_colloc, M_colloc, info_colloc = collocation_solver.solve(Niter=20, l2errBound=1e-4, verbose=False)
        ...
        if M_colloc is not None and U_colloc is not None:
            ...
            print(f"  Mass change: {mass_change:+.2e} ({(mass_change/initial_mass)*100:+.3f}%)")
            print(
                f"  Time: {time_colloc:.2f}s, Converged: {info_colloc.get('converged', False)}, Violations: {violations}"
            )
        else:
...
def analyze_convergence_results(results, problem):
    ...
    successful_methods = [
        method for method in ['fdm', 'hybrid', 'collocation'] if results.get(method, {}).get('success', False)
    ]
    ...
    for method in successful_methods:
        ...
        print(
            f"{result['method_name']:<25} {result['initial_mass']:<12.6f} {result['final_mass']:<12.6f} "
    ...
    if len(successful_methods) >= 2:
        ...
        for method in successful_methods:
            ...
            print(
                f"{result['method_name']}: {quality_rating} (violations: {violations}, min_density: {min_density:.2e})"
            )
...
def create_convergence_comparison_plots(results, problem):
    ...
    for method in successful_methods:
        ...
        ax1.plot(problem.tSpace, mass_evolution, 'o-', label=method_names[method], color=colors[method], linewidth=2)
    ...
    for method in successful_methods:
        ...
        ax2.plot(problem.xSpace, final_density, label=method_names[method], color=colors[method], linewidth=2)
    ...
    for method in successful_methods:
        ...
        ax3.plot(problem.xSpace, final_U, label=method_names[method], color=colors[method], linewidth=2)
    ...
    bars1 = ax6.bar(x_pos - width / 2, violations, width, label='Violations', color='red', alpha=0.7)
    bars2 = ax6.bar(x_pos + width / 2, converged, width, label='Converged', color='green', alpha=0.7)
    ...
    if len(successful_methods) >= 2:
        ...
        scatter = ax7.scatter(
            final_masses, mass_changes, c=[colors[method] for method in successful_methods], s=100, alpha=0.7
        )

        for i, method in enumerate(successful_methods):
            ax7.annotate(
                method_names[method], (final_masses[i], mass_changes[i]), xytext=(5, 5), textcoords='offset points'
            )
    ...
    if successful_methods:
        ...
        im = ax8.imshow(
            ...
            aspect='auto',
            origin='lower',
            extent=[0, problem.T, problem.xmin, problem.xmax],
            cmap='viridis',
        )
    ...
    if len(successful_methods) >= 2:
        ...
        bars = ax9.bar(names, relative_errors, color=[colors[method] for method in successful_methods])
        ...
        for bar, value in zip(bars, relative_errors):
            ax9.text(bar.get_x() + bar.get_width() / 2.0, bar.get_height(), f'{value:.2f}%', ha='center', va='bottom')
        plt.setp(ax9.get_xticklabels(), rotation=45)

    plt.tight_layout()
    plt.savefig(
        '/Users/zvezda/Library/CloudStorage/OneDrive-Personal/code/MFG_PDE/three_method_convergence_comparison.png',
        dpi=150,
        bbox_inches='tight',
    )
    plt.show()
