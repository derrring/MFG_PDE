...
def minimal_convergence_test():
    ...
    base_params = {"xmin": 0.0, "xmax": 1.0, "T": 0.1, "sigma": 0.1, "coefCT": 0.01}  # Very short time
    ...
    for i, config in enumerate(test_configs):
        ...
        print(
            f"Grid: {config['grid']['Nx']}×{config['grid']['Nt']}, "
        ...
        try:
            ...
            # Setup collocation
            collocation_points = np.linspace(problem.xmin, problem.xmax, config['qp']['collocation']).reshape(-1, 1)
            ...
            U_solution, M_solution, info = solver.solve(
                Niter=config['qp']['picard'], l2errBound=1e-2, verbose=False  # Very relaxed
            )
...
def create_minimal_plots(results):
    ...
    bars1 = ax3.bar(x_pos - width / 2, mass_changes, width, label='Mass Change (%)', color='green', alpha=0.7)

    # Secondary y-axis for time
    ax3_twin = ax3.twinx()
    bars2 = ax3_twin.bar(x_pos + width / 2, solve_times, width, label='Solve Time (s)', color='orange', alpha=0.7)
    ...
    for bar, value in zip(bars2, solve_times):
        ax3_twin.text(bar.get_x() + bar.get_width() / 2.0, bar.get_height(), f'{value:.2f}s', ha='center', va='bottom')

    plt.tight_layout()
    plt.savefig(
        '/Users/zvezda/Library/CloudStorage/OneDrive-Personal/code/MFG_PDE/minimal_convergence_test.png',
        dpi=150,
        bbox_inches='tight',
    )
    plt.show()
