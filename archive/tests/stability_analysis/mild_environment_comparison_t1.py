#!/usr/bin/env python3
"""
Mild Environment Comparison Test - T=1 Version
Compares different solver methods under mild/stable parameter conditions with T=1 time horizon.
...
def run_mild_environment_comparison_t1():
    """Run comparison under mild/stable parameter conditions with T=1"""
    print("="*80)
    print("MILD ENVIRONMENT COMPARISON TEST - T=1 VERSION")
    print("="*80)
    print("Testing solver performance under stable, mild parameter conditions (T=1)")
    print("Output: Only figures 1.1 (Mass Evolution) and 1.2 (Density Evolution)")
    
    # Mild/stable problem parameters with T=1 simulation
    mild_params = {
        "xmin": 0.0,
        "xmax": 1.0,
        "Nx": 20,         # Moderate resolution
        "T": 1.0,         # T=1 simulation
        "Nt": 50,         # Good time resolution for T=1
        "sigma": 0.15,    # Mild diffusion
        "coefCT": 0.02,   # Light coupling for stability
    }
    
    print(f"\nMild Environment Parameters (T=1):")
    ...
    # Shared solver settings for T=1 optimal performance
    solver_settings = {
        "num_particles": 250,        # Optimal particles for T=1
        "max_iterations": 12,        # Good iterations for T=1
        "convergence_tolerance": 1e-3,
        "newton_iterations": 6,      # Balanced Newton iterations for T=1
    ...
    print(f"\n{'='*60}")
    print("METHOD 1: HYBRID PARTICLE-FDM (Mild Environment T=1)")
    print(f"{'='*60}")
    ...
    try:
        ...
        U_hybrid, M_hybrid, iters_hybrid, _, _ = hybrid_iterator.solve(
            ...
            solver_settings["convergence_tolerance"]
        )
    ...
    print(f"\n{'='*60}")
    print("METHOD 2: QP PARTICLE-COLLOCATION (Mild Environment T=1)")
    print(f"{'='*60}")
    ...
    try:
        ...
        U_qp, M_qp, solve_info = qp_solver.solve(
            Niter=solver_settings["max_iterations"],
            l2errBound=solver_settings["convergence_tolerance"],
            verbose=False
        )
    ...
    print(f"\n{'='*80}")
    print("MILD ENVIRONMENT ANALYSIS (T=1)")
    print(f"{'='*80}")
    
    analyze_mild_results_t1(results, problem)
    create_mild_comparison_plots_t1(results, problem)
    
    return results

def analyze_mild_results_t1(results, problem):
    """Analyze results from mild environment T=1 test"""
    ...
    print(f"\n--- MILD ENVIRONMENT COMPARISON RESULTS (T=1) ---")
    ...
    # Assessment
    print(f"\n--- MILD ENVIRONMENT ASSESSMENT (T=1) ---")
    
    # Mass conservation
    both_good_mass = (abs(hybrid['mass_change_percent']) < 3 and 
                     abs(qp['mass_change_percent']) < 3)
    if both_good_mass:
        print("✅ Both methods show excellent mass conservation over T=1")
    else:
        print("⚠️  Some mass conservation issues detected")
    
    # Solution quality
    both_clean = (hybrid['violations'] == 0 and qp['violations'] == 0 and
                 hybrid['negative_densities'] == 0 and qp['negative_densities'] == 0)
    if both_clean:
        print("✅ Both methods produce perfectly clean solutions")
    else:
...
def create_mild_comparison_plots_t1(results, problem):
    """Create comparison plots showing only figures 1.1 and 1.2 for T=1"""
    ...
    ax1.plot(problem.tSpace, hybrid['mass_evolution'], 
             'g-o', linewidth=2, markersize=4, label=f'Hybrid ({hybrid["mass_change_percent"]:+.2f}%)')
    ax1.plot(problem.tSpace, qp['mass_evolution'], 
             'r-s', linewidth=2, markersize=4, label=f'QP-Collocation ({qp["mass_change_percent"]:+.2f}%)')
    
    # Add reference lines
    ax1.axhline(y=hybrid['mass_evolution'][0], color='g', linestyle='--', alpha=0.5, 
                label=f'Hybrid Initial: {hybrid["mass_evolution"][0]:.4f}')
    ax1.axhline(y=qp['mass_evolution'][0], color='r', linestyle='--', alpha=0.5,
                label=f'QP Initial: {qp["mass_evolution"][0]:.4f}')
    
    ax1.set_xlabel('Time t')
    ax1.set_ylabel('Total Mass')
    ax1.set_title('Figure 1.1: Mass Evolution Comparison (T=1)')
    ax1.grid(True, alpha=0.3)
    ax1.legend()
    
    # Performance annotation
    performance_text = f"Hybrid: {hybrid['time']:.2f}s | QP: {qp['time']:.2f}s"
    conservation_text = "T=1: Optimal stability conditions"
    ax1.text(0.05, 0.95, f'{performance_text}\n{conservation_text}', 
             transform=ax1.transAxes, fontsize=9, 
             bbox=dict(boxstyle="round,pad=0.3", facecolor="lightgreen", alpha=0.8))
    
    # Figure 1.2: Final Density Comparison
    ax2.plot(problem.xSpace, hybrid['M'][-1, :], 
             'g-', linewidth=2, label='Hybrid Final Density')
    ax2.plot(problem.xSpace, qp['M'][-1, :], 
             'r--', linewidth=2, label='QP-Collocation Final Density')
    
    # Add initial density for reference
    ax2.plot(problem.xSpace, hybrid['M'][0, :], 
             'k:', linewidth=1, alpha=0.7, label='Initial Density')
    
    ax2.set_xlabel('Space x')
    ax2.set_ylabel('Density')
    ax2.set_title('Figure 1.2: Final Density Comparison (T=1)')
    ax2.grid(True, alpha=0.3)
    ax2.legend()
    
    # Quality annotation
    quality_metrics = f"Violations: H={hybrid['violations']}, QP={qp['violations']}"
    ax2.text(0.05, 0.95, f'{quality_metrics}\nT=1: Excellent convergence', 
             transform=ax2.transAxes, fontsize=9,
             bbox=dict(boxstyle="round,pad=0.3", facecolor="lightblue", alpha=0.8))
    
    plt.tight_layout()
    plt.savefig('/Users/zvezda/Library/CloudStorage/OneDrive-Personal/code/MFG_PDE/tests/stability_analysis/mild_environment_comparison_t1.png', 
                dpi=150, bbox_inches='tight')
    plt.show()
    
    print(f"\n✅ Plots saved: mild_environment_comparison_t1.png")
    print("   Figure 1.1: Mass evolution comparison over T=1")
    print("   Figure 1.2: Final density comparison across space (T=1)")

if __name__ == "__main__":
    print("Starting Mild Environment Comparison Test (T=1)...")
    print("Designed to test solver performance under stable, mild conditions with T=1")
    print("Output: Clean 2-panel visualization (figures 1.1 and 1.2 only)")
    
    try:
        results = run_mild_environment_comparison_t1()
        print("\n" + "="*80)
        print("MILD ENVIRONMENT COMPARISON COMPLETED (T=1)")
        print("="*80)
        print("✅ Both methods tested under mild/stable parameter conditions (T=1)")
        print("✅ Clean visualization with figures 1.1 and 1.2 only")
        print("✅ Optimal stability and performance for T=1 simulation")
