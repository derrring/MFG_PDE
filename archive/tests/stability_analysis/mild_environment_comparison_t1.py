...
def analyze_mild_results_t1(results, problem):
    ...
    # Mass conservation
    both_good_mass = abs(hybrid['mass_change_percent']) < 3 and abs(qp['mass_change_percent']) < 3
    if both_good_mass:
...
def create_mild_comparison_plots_t1(results, problem):
    ...
    plt.tight_layout()
    plt.savefig(
        '/Users/zvezda/Library/CloudStorage/OneDrive-Personal/code/MFG_PDE/tests/stability_analysis/mild_environment_comparison_t1.png',
        dpi=150,
        bbox_inches='tight',
    )
    plt.show()
