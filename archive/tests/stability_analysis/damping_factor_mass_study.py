...
def analyze_damping_effects(results, problem):
    ...
    for theta in sorted(successful_results.keys()):
        ...
        print(f"{theta:<8.2f} {early_loss:<+12.3f} {max_dev:<12.3f} {oscillation:<12.3f} {final_change:<+10.3f} {status}")
...
def create_damping_plots(results, problem):
    ...
    for (theta, result), color in zip(sorted(successful_results.items()), colors):
        ax1.plot(problem.tSpace, result['mass_evolution'], 
                color=color, linewidth=2, label=f'θ={theta:.2f}')
    ...
    plt.tight_layout()
    plt.savefig('/Users/zvezda/Library/CloudStorage/OneDrive-Personal/code/MFG_PDE/tests/stability_analysis/damping_factor_mass_study.png', 
