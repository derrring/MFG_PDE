...
def simulate_realistic_first_iteration(problem):
    ...
    print(
        f"    Final time U[-1]: range [{U_current[-1].min():.2f}, {U_current[-1].max():.2f}], norm={np.linalg.norm(U_current[-1]):.2f}"
    )
    print(
        f"    Initial time U[0]: range [{U_current[0].min():.2f}, {U_current[0].max():.2f}], norm={np.linalg.norm(U_current[0]):.2f}"
    )
    print(f"    Total U norm: {np.linalg.norm(U_current):.2f}")

    print(f"  M_current: only initial time set, rest are zeros")
    print(
        f"    Initial time M[0]: range [{M_current[0].min():.2f}, {M_current[0].max():.2f}], norm={np.linalg.norm(M_current[0]):.2f}"
    )
    print(
        f"    Final time M[-1]: range [{M_current[-1].min():.2f}, {M_current[-1].max():.2f}], norm={np.linalg.norm(M_current[-1]):.2f}"
    )
    ...
    print(
        f"    Initial time U[0]: range [{U_new[0].min():.2f}, {U_new[0].max():.2f}], norm={np.linalg.norm(U_new[0]):.2f}"
    )
    print(
        f"    Final time U[-1]: range [{U_new[-1].min():.2f}, {U_new[-1].max():.2f}], norm={np.linalg.norm(U_new[-1]):.2f}"
    )
    print(f"    Total U norm: {np.linalg.norm(U_new):.2f}")

    print(f"  M_new (after FP solve):")
    print(
        f"    Initial time M[0]: range [{M_new[0].min():.2f}, {M_new[0].max():.2f}], norm={np.linalg.norm(M_new[0]):.2f}"
    )
    print(
        f"    Final time M[-1]: range [{M_new[-1].min():.2f}, {M_new[-1].max():.2f}], norm={np.linalg.norm(M_new[-1]):.2f}"
    )
