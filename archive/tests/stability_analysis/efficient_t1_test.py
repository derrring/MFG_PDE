...
def test_efficient_t1():
    ...
    test_configs = [
        {
            ...
            'params': {'xmin': 0.0, 'xmax': 1.0, 'Nx': 30, 'T': 1.0, 'Nt': 20, 'sigma': 0.2, 'coefCT': 0.02},
            'qp_settings': {'particles': 400, 'collocation': 10, 'delta': 0.3, 'order': 2, 'newton': 6, 'picard': 8},
        },
        {
            'name': 'Efficient High-Quality',
            'params': {'xmin': 0.0, 'xmax': 1.0, 'Nx': 40, 'T': 1.0, 'Nt': 25, 'sigma': 0.25, 'coefCT': 0.025},
            'qp_settings': {'particles': 600, 'collocation': 12, 'delta': 0.35, 'order': 2, 'newton': 8, 'picard': 10},
        },
    ]
    ...
    for i, config in enumerate(test_configs):
        ...
        try:
            ...
            U_solution, M_solution, info = solver.solve(
                Niter=config['qp_settings']['picard'], l2errBound=1e-3, verbose=False  # Reduce output for efficiency
            )
    ...
    if len(successful_results) > 0:
        ...
        for result in successful_results:
            ...
            print(
                f"{result['name']:<25} {result['solve_time']/60:<12.1f} {result['mass_change_percent']:<15.3f} {str(result['converged']):<12} {quality:<12}"
            )
        ...
        if len(successful_results) > 1:
            ...
            complexities = [
                r['config']['params']['Nx'] * r['config']['params']['Nt'] * r['config']['qp_settings']['particles']
...
def create_efficient_t1_plots(results):
    ...
    for bar, value in zip(bars, times):
        ax3.text(bar.get_x() + bar.get_width() / 2.0, bar.get_height(), f'{value:.1f}m', ha='center', va='bottom')
    ...
    bars1 = ax6.bar(
        x_pos - width / 2, quality_metrics['Violations'], width, label='Boundary Violations', color='red', alpha=0.7
    )
    bars2 = ax6.bar(
        x_pos + width / 2,
        quality_metrics['Negative Densities'],
        width,
        label='Negative Densities',
        color='orange',
        alpha=0.7,
    )
    ...
    plt.tight_layout()
    plt.savefig(
        '/Users/zvezda/Library/CloudStorage/OneDrive-Personal/code/MFG_PDE/efficient_t1_analysis.png',
        dpi=150,
        bbox_inches='tight',
    )
    plt.show()
