...
class RefactoringValidator:
    ...
    def check_parameter_naming_consistency(self) -> Dict[str, any]:
        ...
        standardized_params = ['max_newton_iterations', 'newton_tolerance', 'max_picard_iterations', 'picard_tolerance']

        deprecated_params = ['NiterNewton', 'l2errBoundNewton', 'Niter_max', 'l2errBoundPicard']
        ...
        consistency_score = (
            standardized_count / (standardized_count + deprecated_count)
            if (standardized_count + deprecated_count) > 0
            else 1.0
        )
    ...
    def check_error_message_quality(self) -> Dict[str, any]:
        ...
        if has_enhanced_exceptions:
            ...
            error_types = ['MFGSolverError', 'MFGConfigurationError', 'MFGConvergenceError', 'MFGValidationError']
        ...
        status = (
            "✅ PASSED"
            if has_enhanced_exceptions and actionable_error_types >= 3 and has_actionable_guidance
            else "⚠️ PARTIAL"
        )
    ...
    def check_documentation_coverage(self) -> Dict[str, any]:
        ...
        doc_coverage_score = (
            examples_with_docs / len(list(examples_dir.glob("*.py")))
    ...
    def check_code_duplication_reduction(self) -> Dict[str, any]:
        ...
        status = (
            "✅ PASSED"
            if has_config_module and has_utils_module and has_factory_module and config_usage_score > 0.3
            else "⚠️ PARTIAL"
        )
    ...
    def check_cyclomatic_complexity(self) -> Dict[str, any]:
        ...
        for file_path in new_files:
            if file_path.exists():
                ...
                complexity_indicators = (
                    content.count('if ') + content.count('for ') + content.count('while ') + content.count('except')
                )
        ...
        avg_complexity = (
            sum(r['ratio'] for r in complexity_results.values()) / len(complexity_results) if complexity_results else 0
        )
