...
class SecurityResultsParser:
                ...
                for line in f:
                    if line.strip():
                        ...
                        finding = {
                            ...
                            'file': finding_data.get('SourceMetadata', {}).get('Data', {}).get('Filesystem', {}).get('file', 'unknown'),
    ...
    def _parse_pip_licenses_results(self) -> List[Dict[str, Any]]:
        ...
        acceptable_licenses = {
            ...
            'PSF', 'Mozilla Public License 2.0 (MPL 2.0)', 'MPL-2.0'
        }
    ...
    def _generate_dependency_recommendations(self, findings: List[Dict[str, Any]]) -> List[str]:
        ...
        if critical_count > 0:
            recommendations.append(f"🚨 {critical_count} critical vulnerabilities found - update dependencies immediately")
...
def main():
    ...
    parser.add_argument('scan_type', choices=['dependency', 'static', 'secrets', 'container', 'license'],
